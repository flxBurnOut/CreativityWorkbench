...
前端已实现项目首页、新建与管理、五阶段创作界面、设置、帮助和四类独立演示。后端仍是空能力清单、HTTP 健康检查与发现、CLI 诊断、MCP 只读 manifest，没有执行能力和生成适配器。
...
正式目标仍为页面 → HTTP → 共享 Runtime → 适配器；本地草稿是获准的前端阶段过渡存储。后续替换存储入口、补充服务端项目与任务接口；现有前端暂存不等于服务端 P1 验收通过。能力清单仍为空。
