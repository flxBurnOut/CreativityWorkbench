...
2. 原任务 succeeded：使用现有结果，不再生成；概念图以 imageState 判断，selected 无需重复采用，unbound / candidate 可由网页或 image_select 最终选用。waiting_external / uncertain：处理原交接。queued / running：继续查询。cancelled 只恢复已经生成的图片，不自发重新生成；failed 先核实错误，不为了消除等待换 ID 重发。
...
6. 概念图成功后网页会直接展示；task_adopt 原 ID 只放入候选，最终选用用网页或 image_select，并读取最新项目版本。修改图须先真实对比；不跨过过期检查或自动覆盖原定稿。详见 [图片状态同步](image-result-sync.md)。
