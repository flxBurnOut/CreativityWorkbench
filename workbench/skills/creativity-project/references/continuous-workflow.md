...
类型切换用 project_update.patch.type，保存各类型独立草稿。显式沿用资料用 workflow_update({action:"inherit",from,brief,content,art,conceptIds,...版本字段})。原内容成为 transfer 适配资料，不替换当前类型小节；对象有新的当前类型 ID，不混用旧任务。恢复用 workflow_update({action:"restore",recordId,...版本字段})；恢复保留原依赖与其他版本。不要用 project_update 写 flow 或 variants。
