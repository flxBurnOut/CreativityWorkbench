...
版本：核心协议 10、29 个工具、Skills 0.7.0。网页默认使用作品预览入口，五阶段为详细编辑。
...
小说默认直接生成或接收完整正文；没有网页文字服务时，按页面交接请求在当前对话创作并写回原项目。视频默认一个镜头，保留横竖屏、时长及现有首帧；按原媒体 handoff 回传，规格不符不能冒充通过。详细人物、美术、素材和分镜仍可按需编辑。3D 仍仅支持前期资料。
