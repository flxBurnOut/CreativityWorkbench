...
export function createRepository(directory) {
  ...
  async function saveWorkspace(workspace, expectedRevision, writeId) {
    ...
    return serial(async () => {
      ...
      validateWorkspace(workspace);
      for (const p of workspace.projects) for (const asset of p.assets) await checkMedia(asset.fileId);
  ...
  async function mutateProject({ operationId, requestHash, projectId, expectedVersion, create = false, legacyHash }, transform) {
    return serial(async () => {
      ...
      validateWorkspace(workspace);
      for (const asset of updated.assets) await checkMedia(asset.fileId);
  ...
  async function putOutputStream(chunks,extension='zip') {
    if(extension!=='zip')throw new ServiceError(400,'invalid_file','仅支持资料 ZIP 流式写入。');
    await initialize();const temporary=file('files','pending-'+randomUUID()+'.tmp');const handle=await open(temporary,'wx');const hash=createHash('sha256');let size=0;
    try{
      for await(const chunk of chunks){size+=chunk.length;if(size>128*1024*1024)throw new ServiceError(413,'too_large','资料包超过 128 MB。');hash.update(chunk);await handle.writeFile(chunk);}
      if(!size)throw new ServiceError(400,'invalid_file','资料包为空。');
      await handle.sync();await handle.close();const fileId=hash.digest('hex')+'.zip';
  ...
  return { root, initialize, loadWorkspace, recoverWorkspace, saveWorkspace, coreReceipt, mutateProject, inbox, readInbox, exportText, readExport, putImage, media, putOutput, output, openStored, fileChunks, putOutputStream, listTasks, task, saveTask, handoff, readHandoff, completeImageHandoff, completeHandoffFile, inspectWebsiteSource, cacheStats:()=>({tasks:taskCache.stats(),sources:{...inspectedSources.stats(),validations:sourceValidations}}) };
}
