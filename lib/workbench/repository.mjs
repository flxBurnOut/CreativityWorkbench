...
import { outputId, validateVideo, validateWebsite, projectOutputIds } from './output-contract.mjs';
...
export function validateWorkspace(ws) {
  ...
  for (const p of ws.projects) {
    ...
    requireValid(optionalAsset(p.coverAssetId) && Array.isArray(p.references) && p.references.length <= 100 && p.references.every(r => record(r) && identifier(r.id) && assetIds.has(r.assetId) && string(r.purpose, 4000)));
    ...
    try { if (p.video) validateVideo(p.video); if (p.website) validateWebsite(p.website, assetIds); }
...
export function createRepository(directory) {
  ...
  async function coreReceipt(operationId, requestHash) {
    const saved = await loadWorkspace();
    const prior = saved.coreReceipts?.[digest(operationId)];
    if (prior && prior.requestHash !== requestHash) throw new ServiceError(409, 'conflict', '此 requestId 已用于另一项操作，请勿改变重试参数。');
    return prior?.result;
  }
  async function mutateProject({ operationId, requestHash, projectId, expectedVersion, create = false }, transform) {
    return serial(async () => {
      await initialize();
      const old = await loadWorkspace();
      const prior = old.coreReceipts?.[digest(operationId)];
      if (prior) {
        if (prior.requestHash !== requestHash) throw new ServiceError(409, 'conflict', '此 requestId 已用于另一项操作，请勿改变重试参数。');
  ...
  async function readInbox(projectId, name) {
    requireValid(/^[a-zA-Z0-9_-]{1,80}\.(png|jpg|jpeg|webp|mp4|wav|mp3|m4a)$/.test(name), '请使用 inbox 内的简单文件名，如 reference.png；不接受任意文件路径。');
    const target = join(await inbox(projectId), name);
    try {
      const actual = await realpath(target);
      if ((process.platform === 'win32' ? actual.toLowerCase() !== target.toLowerCase() : actual !== target)) throw new ServiceError(400, 'invalid_file', '导入文件不能链接到其他目录。');
  ...
  async function listTasks() {
    ...
    return Promise.all(names.map(n => json(file('tasks', n))));
  }
  ...
  async function readHandoff(id, extension = 'png') {
    ...
    const directory = file('handoff', id);
    try {
      // Accept only the assigned file, never paths supplied by model text or arbitrary reply URLs.
      const target = join(directory, 'result.' + extension);
      if ((await realpath(target)).toLowerCase() !== target.toLowerCase()) throw new ServiceError(400, 'invalid_image', '任务结果不能链接到其他目录。');
      if ((await stat(target)).size > (extension === 'png' ? 20 : 128) * 1024 * 1024) throw new ServiceError(413,'too_large','交接文件超过接收大小，请减小后写回。');
      return { bytes: await readFile(target) };
    } catch (e) {
      if (e instanceof ServiceError) throw e;
      if (e.code !== 'ENOENT') throw e;
      const failure = await json(join(directory, 'error.json'), null);
      return failure ? { failed: true } : null;
    }
  }
  return { root, initialize, loadWorkspace, saveWorkspace, coreReceipt, mutateProject, inbox, readInbox, exportText, readExport, putImage, media, putOutput, output, listTasks, task, saveTask, handoff, readHandoff };
}
