...
import { mkdir, readFile, rename, writeFile, readdir, realpath, stat, link, unlink, open } from 'node:fs/promises';
...
export function createRepository(directory) {
  ...
  async function checkSources(p) {
    for(const branch of [p,...Object.values(p.variants||{})])for(const source of [branch.websiteSource,branch.websiteSourceCandidate].filter(Boolean))if(!inspectedSources.has(source.fileId)){inspectWebsiteZip(await output(source.fileId));inspectedSources.set(source.fileId,true);}
  }
  ...
  async function completeImageHandoff(id, bytes) {
    requireValid(identifier(id) && Buffer.isBuffer(bytes) && bytes.length>0 && bytes.length<=20*1024*1024, '图片交接输入无效或超过 20 MB。');
    ...
    await assignedDirectory('handoff',id);
    const target=file('handoff',id,'result.png');
    return locked(target,async()=>{
      const sameExisting=async()=>{
        const actual=await assignedFile('handoff',id,'result.png');
