...
import { mkdir, readFile, rename, readdir, realpath, stat, link, unlink, open } from 'node:fs/promises';
...
import { projectMediaIds } from './media-references.mjs';
...
import {validCraftFileHeader,CRAFT_HEADER_BYTES} from './craft-file-format.mjs';
// Native image cache is process-wide and independent of the JavaScript heap.
sharp.cache({memory:32,files:0,items:32});sharp.concurrency(2);
...
export function validateWorkspace(ws) {
  ...
  for (const p of ws.projects) {
    ...
    try { validateKnowledge(p.knowledge); } catch(e) { throw new ServiceError(400, 'invalid_input', e.message); }
...
export function createRepository(directory) {
  ...
  const taskCache = boundedCache(64,8*1024*1024);
  const inspectedSources = boundedCache(128);
  let sourceValidations = 0;
  function inspectWebsiteSource(bytes) {
    if (!Buffer.isBuffer(bytes) || !bytes.length || bytes.length > 128*1024*1024) throw new ServiceError(400,'invalid_source','源码 ZIP 须为非空实际文件，且不超过 128 MB。');
    // The key is calculated from the actual bytes, never supplied by a caller.
    // Retain only small metadata; decompressed members must be collectible.
    const fileId = digest(bytes) + '.zip';
    const cached = inspectedSources.get(fileId);
    if (cached) return {...cached};
    const {entryCount,previewPath} = inspectWebsiteZip(bytes);
    const inspected = {fileId,entryCount,previewPath};
    sourceValidations++;
    inspectedSources.set(fileId,inspected);
    return {...inspected};
  }
  async function checkSources(p) {
    for(const branch of [p,...Object.values(p.variants||{})])for(const source of [branch.websiteSource,branch.websiteSourceCandidate].filter(Boolean))if(!inspectedSources.has(source.fileId))inspectWebsiteSource(await output(source.fileId));
  }
  ...
  async function atomic(path, data) {
    ...
    let handle;
    try {
      // Successful exclusive open proves this invocation owns the temporary
      // file, including when a later write fails with ENOSPC.
      handle = await open(temp, 'wx');
      await handle.writeFile(data); await handle.close();
  ...
  async function saveWorkspace(workspace, expectedRevision, writeId) {
    ...
    return serial(async () => {
      ...
      validateWorkspace(workspace);
      for (const p of workspace.projects) for (const id of projectMediaIds(p)) await checkMedia(id);
      for (const p of workspace.projects) for (const id of projectOutputIds(p)) await checkOutput(id);
  ...
  async function mutateProject({ operationId, requestHash, projectId, expectedVersion, create = false, legacyHash }, transform) {
    return serial(async () => {
      ...
      validateWorkspace(workspace);
      for (const id of projectMediaIds(updated)) await checkMedia(id);
      for (const id of projectOutputIds(updated)) await checkOutput(id);
  ...
  async function restoreImage(bytes, id) {
    if (!fileIdentifier(id) || !Buffer.isBuffer(bytes) || !bytes.length || bytes.length > 32 * 1024 * 1024 || digest(bytes) !== id) throw new ServiceError(400, 'invalid_backup', '备份图案的图片标识、大小或内容校验不一致。');
    try {
      const input = sharp(bytes, {limitInputPixels: 40000000, failOn: 'warning', animated: false});
      const metadata = await input.metadata();
      if (metadata.format !== 'png' || (metadata.pages ?? 1) > 1) throw Error('format');
      // Decode for validity without keeping a full-resolution raw bitmap or
      // re-encoding the persisted bytes; model references depend on this hash.
      await input.resize(1, 1).raw().toBuffer();
    } catch { throw new ServiceError(400, 'invalid_backup', '备份图案不是可解码的静态 PNG。'); }
    await initialize(); await atomic(file('media', id + '.png'), bytes);
    return {fileId: id, mime: 'image/png'};
  }
  // Immutable media was validated at import. Saving text only needs to verify
  // the reference, without allocating every image/video in the workspace again.
  async function checkFile(path, code, message) {
    try {
      await locked(path, async () => {
        const info = await stat(path);
        if (!info.isFile()) throw new Error('not a file');
      });
    } catch { throw new ServiceError(404, code, message); }
  }
  async function checkMedia(id) {
    if (!fileIdentifier(id)) throw new ServiceError(400, 'invalid_input', '图片标识无效。');
    await checkFile(file('media', id + '.png'), 'missing_image', '原始图片不存在，请重新上传。');
  }
  async function checkOutput(id) {
    if (!outputId(id)) throw new ServiceError(400, 'invalid_file', '成品文件标识无效。');
    await checkFile(file('files', id), 'missing_file', '成品文件不存在，请恢复备份或重新生成。');
  }
  async function listTasks({summary=false,projectId}={}) {
    await initialize();
    ...
    for(const name of names) {
      ...
      const raw=cached?.signature===signature?cached.raw:await locked(path,()=>readFile(path,'utf8'));
      if(cached?.signature!==signature)taskCache.set(name,{signature,raw},raw.length*2);
      let value;try{value=JSON.parse(raw);}catch{throw new ServiceError(500,'storage_error','任务记录无法读取，请保留数据目录。');}
      if(projectId&&value.projectId!==projectId)continue;
      if(summary){const {snapshot,requestHash:_hash,...small}=value;value={...small,workType:small.workType||snapshot?.type,targetName:snapshot?.video?.shots.find(s=>s.id===small.args?.objectId)?.title};}
      result.push(value);
    }
    return result;
  }
  ...
  async function putOutput(bytes, extension) {
    if (!['mp4','wav','html','zip','srt','blend','glb'].includes(extension) || !Buffer.isBuffer(bytes) || bytes.length > (['blend','glb'].includes(extension)?32:128) * 1024 * 1024 || (!bytes.length && extension !== 'srt')) throw new ServiceError(400, 'invalid_file', '成品文件格式或大小无效。');
    if(['blend','glb'].includes(extension)&&!validCraftFileHeader(bytes,extension,bytes.length))throw new ServiceError(400,'invalid_file','三维成品格式无效，不能使用改后缀的文件。');
  ...
  async function handoff(id, prompt, images, extension = 'png', context = {}) {
    if (!['png','mp4','wav','zip'].includes(extension)) throw new ServiceError(400, 'invalid_file', '交接格式无效。');
    ...
    await atomic(join(directory, 'request.json'), JSON.stringify({ mode:'complete-existing-task', taskId: id, projectId:context.projectId, kind:context.kind, objectId:context.objectId, prompt, inputImages: inputs, output, failureFile: join(directory, 'error.json') }, null, 2));
    return { directory, requestPath: join(directory, 'request.json'), output };
  }
  async function openStored(id,mediaFile=false) {
    if(mediaFile?!fileIdentifier(id):!outputId(id))throw new ServiceError(400,'invalid_file','文件标识无效。');
    let handle;
    try{handle=await open(mediaFile?file('media',id+'.png'):file('files',id),'r');const info=await handle.stat();if(!info.isFile())throw new Error('not-file');return {handle,size:info.size};}
    catch{await handle?.close();throw new ServiceError(404,'missing_file','素材文件不存在，请恢复备份。');}
  }
  async function* fileChunks(id,mediaFile=false) {
    const {handle}=await openStored(id,mediaFile);
    try{yield* handle.createReadStream({highWaterMark:64*1024,autoClose:false});}finally{await handle.close();}
  }
  async function putOutputStream(chunks,extension='zip') {
    if(!['zip','blend','glb'].includes(extension))throw new ServiceError(400,'invalid_file','仅支持资料 ZIP 和三维资产流式写入。');
    await initialize();const temporary=file('files','pending-'+randomUUID()+'.tmp');const handle=await open(temporary,'wx');const hash=createHash('sha256');let size=0,header=Buffer.alloc(0);
    try{
      for await(const chunk of chunks){
        if(!(chunk instanceof Uint8Array))throw new ServiceError(400,'invalid_file','成品数据须为二进制文件流。');
        const limitMiB=extension==='zip'?128:32;
        size+=chunk.length;if(size>limitMiB*1024*1024)throw new ServiceError(413,'too_large',`成品文件超过 ${limitMiB} MB。`);
        if(header.length<CRAFT_HEADER_BYTES)header=Buffer.concat([header,chunk.subarray(0,CRAFT_HEADER_BYTES-header.length)]);
        hash.update(chunk);await handle.writeFile(chunk);
      }
      if(!size)throw new ServiceError(400,'invalid_file','成品文件为空。');
      if(['blend','glb'].includes(extension)&&!validCraftFileHeader(header,extension,size))throw new ServiceError(400,'invalid_file','三维成品格式无效，未保存不完整文件。');
      await handle.sync();await handle.close();const fileId=hash.digest('hex')+'.'+extension;
      await locked(file('files',fileId),async()=>{try{await link(temporary,file('files',fileId));}catch(e){if(e.code!=='EEXIST')throw e;}});
      return {fileId};
    }finally{await handle.close().catch(()=>{});await unlink(temporary).catch(()=>{});}
  }
  async function completeHandoffFile(id, bytes, extension='png') {
    requireValid(identifier(id) && ['png','zip'].includes(extension) && Buffer.isBuffer(bytes) && bytes.length>0 && bytes.length<=(extension==='png'?20:128)*1024*1024, '交接文件无效或超过接收大小。');
    // Publish complete bytes atomically without replacing an existing result,
    // including when an external writer or another completion call races us.
    await assignedDirectory('handoff',id);
    const target=file('handoff',id,'result.'+extension);
    return locked(target,async()=>{
      const sameExisting=async()=>{
        const actual=await assignedFile('handoff',id,'result.'+extension);
        const info=await stat(actual);
        if(!info.isFile()||info.size!==bytes.length||!(await readFile(actual)).equals(bytes))throw new ServiceError(409,'handoff_conflict','原任务已有不同的交接文件，未覆盖；请查询原任务并核对结果。');
        return {alreadyWritten:true};
      };
      try { return await sameExisting(); } catch(e) { if(e.code!=='ENOENT')throw e; }
      const temp=target+'.'+randomUUID()+'.tmp';
      let handle;
      try {
        handle=await open(temp,'wx');await handle.writeFile(bytes);await handle.close();
        try {await link(temp,target);return {alreadyWritten:false};}
        catch(e) {if(e.code==='EEXIST')return sameExisting();throw e;}
      } finally {if(handle){await handle.close().catch(()=>{});await unlink(temp).catch(()=>{});}}
    });
  }
  const completeImageHandoff=(id,bytes)=>completeHandoffFile(id,bytes,'png');
  async function readHandoff(id, extension = 'png') {
    if (!['png','mp4','wav','zip'].includes(extension)) return null;
  ...
  return { root, initialize, loadWorkspace, recoverWorkspace, saveWorkspace, coreReceipt, mutateProject, inbox, readInbox, exportText, readExport, putImage, media, restoreImage, putOutput, output, openStored, fileChunks, putOutputStream, listTasks, task, saveTask, handoff, readHandoff, completeImageHandoff, completeHandoffFile, inspectWebsiteSource, cacheStats:()=>({tasks:taskCache.stats(),sources:{...inspectedSources.stats(),validations:sourceValidations}}) };
}
