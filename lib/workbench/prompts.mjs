...
import { activeArtPrompt } from './art-prompt.mjs';
...
export const PROMPT_VERSION = 'lingnan-2026-09-10-v7';
const rules = [
  ...
  { full: '本次明确修改只在其作用范围内优先，其余项目设定继续保持。参考资料只影响指定方面，不自动改写文化背景、对象身份或既定限制。美术风格以当前生效提示词为准；重大冲突应指出，不静默选择。', visual: '只修改指定项，参考仅影响指定方面，保持其余设定。' },
...
export const TYPE_RULES = {
  ...
  craft: '文化依据落实到三维形体、材料、结构意图与使用情境。craft_generate 可生成受限器具与建筑构件的实际 Blender 网格；文字、概念图和设计资料包仍仅为参考，不冒充模型。程序构造为原创数字设计，不承诺任意雕塑、实物复原或制造可行性；网页只查看与下载。',
};
const IMAGE_USE = {
...
export function artContext(art) {
  return { fullPrompt: activeArtPrompt(art) };
}
export function visualArt(art) {
  return activeArtPrompt(art);
}
