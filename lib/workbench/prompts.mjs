...
export const PROMPT_VERSION = 'lingnan-2026-09-09-v3';
const rules = [
...
export function websitePromptBasis(p) {
  return JSON.stringify([PROMPT_VERSION, p.type, p.title, p.idea, p.brief, p.culture, knowledgeText(p), p.content.website, p.art, p.delivery.notes, p.requests[4], assetRoles(p, websiteAssetIds(p)), sourceValue(p,'websiteSource')||p.website?.zipFileId || null]);
}
