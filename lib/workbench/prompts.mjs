...
import { imageInputIds, relatedShotContext, relatedShotConcepts, objectSourceKeys, sourceValue } from './workflow.mjs';
export const PROMPT_VERSION = 'lingnan-2026-09-08-v2';
const rules = [
...
export function imagePrompt(p, args, kind = 'image') {
  ...
  const prompt = [
    ...
    edit ? `图片 1 是必须基于其编辑的原图。本次修改：${c?.revisionRequest}。除明确修改项外，保留原图主体辨识、构图、光线和其他设定。${args.newStyle ? '已明确允许改用当前美术风格。' : '当前美术要求与原图不同也不自动换风格。'}若文化背景与原图冲突而无法局部修改，应报告需要重新生成，不能假装保持全部原设定。` : '按对象设定清楚呈现画面，不增加故事、品牌或无依据文化符号。',
...
export function videoPromptBasis(p, s) {
  return JSON.stringify([PROMPT_VERSION, p.type, p.culture || p.brief || p.idea, p.art, s.title, s.visual, s.camera, s.revision, s.duration, p.video?.ratio,
