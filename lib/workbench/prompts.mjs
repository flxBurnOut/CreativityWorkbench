...
export function websitePromptBasis(p) {
  return JSON.stringify([PROMPT_VERSION, p.type, p.title, p.idea, p.brief, p.culture, knowledgeText(p), p.content.website, p.art, p.delivery.notes, p.requests[4], assetRoles(p, websiteAssetIds(p)), fingerprint(selectedTransfers(p)),sourceValue(p,'transferSources'),sourceValue(p,'websiteSource')||p.website?.zipFileId || null]);
}
