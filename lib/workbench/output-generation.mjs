...
export async function runOutputTask(task,{repository,env,fetchImpl,signal,update},culturalRules) {
  ...
  if(task.kind==='video-plan') {
    ...
    const context={title:p.title,idea:p.idea,brief:p.brief,culture:p.culture,content:p.content[p.type],art:p.art,delivery:p.delivery,instruction:p.requests[4],action:task.args.action,existing:p.video,assets:assetRoles(p,selectedIds).filter(a=>a.role==='output')};
