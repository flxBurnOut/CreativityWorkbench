...
const defaults = { IMAGE_API_BASE_URL: 'https://api.openai.com/v1', IMAGE_API_MODEL: 'gpt-image-2', VIDEO_API_BASE_URL: 'https://api.dev.runwayml.com/v1' };
...
function validate(input) {
  ...
  for (const [key, value] of Object.entries(input)) {
    if (typeof value !== 'string' || value.length > 4096 || ['\r', '\n', '\0'].some(character => value.includes(character))) throw new ServiceError(400, 'invalid_settings', '配置内容格式无效。');
