...
export function createCoreService(repository, tasks, { env, fetchImpl }) {
  ...
  async function call(name, raw) {
    ...
    if (name === 'workbench_status') return { ...runtimeIdentity(repository), providers: generationStatus(env), tools: Object.keys(coreTools),resources:{memory:process.memoryUsage(),caches:repository.cacheStats?.(),mediaProcessLimitMiB:1024},localConversation: true, scope: '项目、文字、主题视觉资产、概念图、单镜头视频、完整正文与媒体向网站交接；网站实现由当前 agent 完成；3D 未实现' };
    ...
    if (name === 'task_list') {
      ...
      return { tasks: (await tasks.list(input.projectId)).map(t => ({ id: t.id, projectId: t.projectId, kind: t.kind, objectId:t.args?.objectId, workType:t.workType, status: t.status, dismissed: Boolean(t.dismissed), retryOf: t.retryOf, supersededBy:t.supersededBy,executionStatus:t.executionStatus,imageState:t.imageState,resultAssetId:t.result?.asset?.id,resultFileId:t.result?.asset?.fileId, recoveredAfterCancel: t.recoveredAfterCancel, createdAt: t.createdAt, updatedAt: t.updatedAt, targetName: t.targetName, error: t.error, note: t.note, dispatch: t.dispatch, handoffContract:existingImageHandoff(t).handoffContract })) };
    }
