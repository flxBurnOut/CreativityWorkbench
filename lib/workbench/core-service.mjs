...
export function createCoreService(repository, tasks, { env, fetchImpl }) {
  ...
  async function call(name, raw) {
    ...
    if (name === 'workbench_status') return { ...runtimeIdentity(repository), providers: generationStatus(env), tools: Object.keys(coreTools), localConversation: true, scope: '项目、文字、美术、图像、视频、静态网站；3D 未实现' };
    ...
    if (name === 'task_start') {
      ...
      if (['content', 'novel', 'video-plan', 'website', 'image'].includes(input.kind)) args.action ??= 'generate';
      if (['image', 'cover', 'video-shot', 'video-audio'].includes(input.kind)) {
        args.provider ??= input.kind === 'video-audio' ? 'workbuddy' : ['image','cover'].includes(input.kind) ? generationStatus(env).images.provider : generationStatus(env).video.provider;
    ...
    const hash = name === 'task_adopt' ? digest({ taskId: input.taskId }) : operationHash(input);
