...
export function createCoreService(repository, tasks, { env, fetchImpl }) {
  ...
  async function call(name, raw) {
    ...
    if (name === 'task_list') {
      ...
      return { tasks: (await tasks.list(input.projectId)).map(t => ({ id: t.id, projectId: t.projectId, kind: t.kind, objectId:t.args?.objectId, workType:t.workType, status: t.status, dismissed: Boolean(t.dismissed), retryOf: t.retryOf, recoveredAfterCancel: t.recoveredAfterCancel, createdAt: t.createdAt, updatedAt: t.updatedAt, targetName: t.targetName, error: t.error, note: t.note, dispatch: t.dispatch, handoffContract:existingImageHandoff(t).handoffContract })) };
    }
