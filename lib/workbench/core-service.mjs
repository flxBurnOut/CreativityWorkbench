...
export function createCoreService(repository, tasks, { env, fetchImpl }) {
  ...
  async function deliver(input) {
    ...
    if (input.format === 'all') {
      for (const a of p.assets) {
        await available({ role: 'image', name: a.name, assetId: a.id, fileId: a.fileId, path: join(repository.root, 'media', a.fileId + '.png'), url: '/v1/media/' + a.fileId }, () => repository.media(a.fileId));
      }
      for (const s of p.video?.shots || []) {
        if (s.clip) files.push(file(s.clip.fileId, 'clip:' + s.id, s.clip.source !== shotSource(s, p.video.ratio,p)));
      ...
      if(p.websiteSource)files.push(file(p.websiteSource.fileId,'website-source'));
      if(p.websiteSourceCandidate)files.push(file(p.websiteSourceCandidate.fileId,'website-source-candidate'));
    ...
    return { projectId: p.id, projectVersion, files, missing, partial: missing.length > 0, limitations: p.website?.spec.limitations || [], note: missing.length ? '部分文件缺失，可交付文件仍可下载；请按 missing 列表恢复备份或重新生成。' : files.length ? '实际文件已可读取；stale=true 表示依据已改变，需重新生成。' : '还没有可交付文件。' };
  }
  async function call(name, raw) {
    ...
    if (name === 'task_adopt') {
      ...
      return repository.mutateProject({ operationId, requestHash: hash, legacyHash: digest(input), projectId: task.projectId, expectedVersion: input.expectedVersion }, project => {
      ...
      try {
        ...
        return await repository.mutateProject(options, project => {
          ...
          if (isImage) {
            ...
            s[input.role] = { fileId: stored.fileId, duration: stored.duration, source: input.role === 'clip' ? shotSource(s, next.video.ratio,next) : audioSource(s,next) };
          }
