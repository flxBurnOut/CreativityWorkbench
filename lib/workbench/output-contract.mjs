...
export const outputId = v => typeof v === 'string' && /^[a-f0-9]{64}\.(mp4|wav|html|zip|srt|blend|glb)$/.test(v);
...
export function validateWebsitePrompt(request, assetIds) {
  ...
  check(request.baseFileId===undefined||(outputId(request.baseFileId)&&request.baseFileId.endsWith('.zip')),'网站修改依据文件无效。');
  return request;
}
export function projectOutputIds(p) {
  const own=q=>[...(q.video?.shots || []).flatMap(s => [s.clip?.fileId,s.audio?.fileId]),q.video?.music?.fileId,q.video?.final?.fileId,q.video?.final?.subtitleFileId,q.website?.previewFileId,q.website?.zipFileId,q.websiteRequest?.bundleFileId,q.websiteRequest?.baseFileId,q.websiteSource?.fileId,q.websiteSourceCandidate?.fileId,q.designPackage?.fileId,q.craftAsset?.blendFileId,q.craftAsset?.glbFileId,q.craftAsset?.texture?.sourceBlendFileId,q.craftAsset?.texture?.sourceGlbFileId,...(q.transfers??(q.transfer?[q.transfer]:[])).flatMap(t=>(t.media||[]).map(m=>m.fileId))];
  const history=(p.flow?.records||[]).flatMap(r=>r.target==='craftAsset'?[r.value?.blendFileId,r.value?.glbFileId,r.value?.texture?.sourceBlendFileId,r.value?.texture?.sourceGlbFileId]:['websiteSource','designPackage','videoFinal'].includes(r.target)?[r.value?.fileId,r.value?.subtitleFileId]:r.target==='websiteRequest'?[r.value?.bundleFileId,r.value?.baseFileId]:r.target.startsWith('shot:')?[r.value?.clip?.fileId,r.value?.audio?.fileId]:[]);
