...
export function projectOutputIds(p) {
  const own=q=>[...(q.video?.shots || []).flatMap(s => [s.clip?.fileId,s.audio?.fileId]),q.video?.music?.fileId,q.video?.final?.fileId,q.video?.final?.subtitleFileId,q.website?.previewFileId,q.website?.zipFileId,q.websiteRequest?.bundleFileId,q.websiteRequest?.baseFileId,q.websiteSource?.fileId,q.websiteSourceCandidate?.fileId,q.designPackage?.fileId,...(q.transfers??(q.transfer?[q.transfer]:[])).flatMap(t=>(t.media||[]).map(m=>m.fileId))];
  const history=(p.flow?.records||[]).flatMap(r=>['websiteSource','designPackage','videoFinal'].includes(r.target)?[r.value?.fileId,r.value?.subtitleFileId]:r.target==='websiteRequest'?[r.value?.bundleFileId,r.value?.baseFileId]:r.target.startsWith('shot:')?[r.value?.clip?.fileId,r.value?.audio?.fileId]:[]);
