...
import {importWebsiteSource,inspectWebsiteZip,SOURCE_PREVIEW_CSP,sourceMime} from './workflow-delivery.mjs';
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === 'GET' && pathname === '/health') {
        ...
        const {entries}=inspectWebsiteZip(await repository.output(id));const bytes=entries[path];
        if(!bytes)throw new ServiceError(404,'missing_file','源码包中没有此文件。');
        response.writeHead(200,{'Content-Type':sourceMime(path),'Content-Security-Policy':SOURCE_PREVIEW_CSP,'X-Content-Type-Options':'nosniff','Referrer-Policy':'no-referrer','Access-Control-Allow-Origin':'*'});response.end(bytes);
