...
const fileMime = {mp4:'video/mp4',wav:'audio/wav',html:'text/html; charset=utf-8',zip:'application/zip',srt:'text/plain; charset=utf-8',blend:'application/octet-stream',glb:'model/gltf-binary'};
  ...
  async function handleRequest(request, response) {
    ...
    try {
      ...
      if (request.method === 'GET' && pathname === '/health') {
        ...
        send(200, {...generationStatus(env),craft:await craftGenerationStatus(env)});
      ...
      } else if (request.method === 'POST' && /^\/v1\/media\/restore\/[a-f0-9]{64}$/.test(pathname)) {
        if (request.headers['content-type'] !== 'image/png') throw new ServiceError(415, 'invalid_backup', '备份图案须为 PNG。');
        if(mediaActive>=2)throw new ServiceError(429,'busy','正在恢复图片，请稍后重试。');mediaActive++;
        try {send(201, await repository.restoreImage(await readBody(request, 32 * 1024 * 1024), pathname.split('/').pop()));} finally {mediaActive--;}
      ...
      } else if (request.method === 'POST' && /^\/v1\/files\/(mp4|wav|restore\/[a-f0-9]{64}\.(mp4|wav|html|zip|srt|blend|glb))$/.test(pathname)) {
        ...
        try {
          ...
          const bytes=await readBody(request,(['blend','glb'].includes(extension)?32:128)*1024*1024);
      ...
      } else if (request.method === 'GET' && /^\/v1\/files\/[a-f0-9]{64}\.(mp4|wav|html|zip|srt|blend|glb)$/.test(pathname)) {
        ...
        const headers={'Content-Type':fileMime[ext],'Cache-Control':'private, max-age=3600','X-Content-Type-Options':'nosniff','Accept-Ranges':'bytes',...(ext==='html'?{'Content-Security-Policy':SITE_CSP,'Referrer-Policy':'no-referrer'}:{}),...(['zip','srt','blend'].includes(ext)?{'Content-Disposition':`attachment; filename="${ext==='zip'?'website':ext==='blend'?'asset':'captions'}.${ext}"`}:{})};
        try {
