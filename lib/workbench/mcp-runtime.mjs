...
export function createRuntimeClient({ env = process.env, autoStart = false, fetchImpl = fetch } = {}) {
  ...
  async function health() {
    ...
    if (!response.ok || value.workbench !== 'creativity-workbench' || value.coreProtocol !== 1 || value.dataId !== dataId) throw new ServiceError(503, 'runtime_mismatch', '端口上的 Runtime 版本或数据目录不匹配。请停止旧版 Runtime，按连接说明重启；不会启动第二个数据写入器。');
    return true;
  }
  ...
  async function call(name, input) {
    ...
    try {
      response = await fetchImpl(new URL('/v1/core/' + name, base), {
        ...
        signal: AbortSignal.timeout(name === 'media_import' ? 195000 : 25000), redirect: 'error',
      });
