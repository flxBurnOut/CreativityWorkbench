...
export async function openRuntimeLog(directory) {
  const path=join(directory,'runtime.log');
  // Rotate only on launch; a healthy shared Runtime keeps its existing handle.
  if((await stat(path).catch(()=>({size:0}))).size>=5*1024*1024) {
    for(let index=2;index>=0;index--){
      const source=index?path+'.'+index:path,target=path+'.'+(index+1);
...
export function createRuntimeClient({ env = process.env, autoStart = false, fetchImpl = fetch } = {}) {
  ...
  async function start() {
    ...
    const log = await openRuntimeLog(directory);
    let launchError,launched,ready=false;
    try {
      ...
      for (let i = 0; i < 40; i++) {
        if (await health()) {ready=true;return;}
        if (launchError) break;
        await new Promise(r => setTimeout(r, 200));
      }
    } finally {
      // A failed startup is ours to stop; never terminate a pre-existing listener.
      if(!ready&&launched?.pid&&launched.exitCode===null)launched.kill('SIGTERM');
      await log.close();
    }
