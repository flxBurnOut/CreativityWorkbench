  ...
  async function refreshExternal(task) {
    ...
    try {
      ...
      return serial(async () => {
        ...
        return repository.saveTask({ ...current, ...(current.status === 'cancelled' ? { recoveredAfterCancel: true, dismissed: false } : {}), status: 'succeeded', error: undefined, code: undefined, note: undefined, updatedAt: Date.now(), result: { asset: { id: randomUUID(), name: task.kind === 'cover' ? task.snapshot.title + '封面' : c.name, fileId: stored.fileId, source: { ...task.provenance, model: 'WorkBuddy 实际使用的生图能力' } } } });
      });
    } catch (e) {
