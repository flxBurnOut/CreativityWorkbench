...
export function createTaskManager(repository, { env = process.env, fetchImpl = fetch, onError = () => {} } = {}) {
  ...
  async function submit(input) {
    ...
    return serial(async () => {
      ...
      if (unfinished.some(t => t.projectId === input.projectId && t.snapshot?.type === project.type && (t.kind === input.kind) && (t.args.objectId ?? '') === (input.args.objectId ?? ''))) throw new ServiceError(409, 'pending_task', '此对象或阶段已有未结束请求，请先查看任务结果或取消等待。');
      ...
      const task = { ...input, snapshot: project, workType: project.type, requestHash: hash, status: 'queued', createdAt: Date.now(), updatedAt: Date.now(), dismissed: false };
  ...
  return { ready, submit, list, get, cancel, dismiss, stop };
}
