...
export function createTaskManager(repository, { env = process.env, fetchImpl = fetch, onError = () => {} } = {}) {
  ...
  async function update(id, patch) {
    return serial(async () => {
      ...
      if (!current || (current.status === 'cancelled' && patch.dismissed === undefined)) return current;
  ...
  async function submit(input) {
    ...
    return serial(async () => {
      ...
      const unfinished = (await repository.listTasks()).filter(t => ['queued','running','waiting_external','waiting_provider','uncertain'].includes(t.status));
  ...
  async function refreshExternal(task) {
    if (['video-shot','video-audio'].includes(task.kind)) {
      if (!stopped && active+refreshing.size<2 && ['waiting_external','waiting_provider','uncertain'].includes(task.status) && !(task.status==='waiting_external'&&task.dispatch==='pending') && (task.handoff || task.providerJob) && !refreshing.has(task.id) && Date.now()-(task.lastPolledAt||0)>=5000) {
    ...
    if (!['waiting_external','uncertain'].includes(task.status) || !task.handoff) return task;
    try {
      ...
      return serial(async () => {
        ...
        if (!['waiting_external','uncertain'].includes(current.status)) return current;
        return repository.saveTask({ ...current, status: 'succeeded', error: undefined, code: undefined, note: undefined, updatedAt: Date.now(), result: { asset: { id: randomUUID(), name: task.kind === 'cover' ? task.snapshot.title + '封面' : c.name, fileId: stored.fileId, source: { ...task.provenance, model: 'WorkBuddy 实际使用的生图能力' } } } });
      });
    } catch (e) {
  ...
  async function cancel(id) {
    ...
    return serial(async () => {
      ...
      const result = await repository.saveTask({ ...task, status: 'cancelled', updatedAt: Date.now(), error: '已停止接收结果。已发送请求可能仍产生费用；WorkBuddy 中的任务需在当地取消。' });
      controllers.get(id)?.abort(); return publicTask(result);
    });
  }
  async function dismiss(id) { await ready(); return publicTask(await update(id, { dismissed: true })); }
