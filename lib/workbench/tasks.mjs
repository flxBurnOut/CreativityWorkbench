...
export function createTaskManager(repository, { env = process.env, fetchImpl = fetch, onError = () => {} } = {}) {
  ...
  async function refreshExternal(task,explicit=false) {
  ...
  async function list(projectId) {
    ...
    for (const task of selected) {
      ...
      result.push(publicTask(await refreshExternal(full),projects.find(p=>p.id===task.projectId)));
    }
    return result;
  }
  async function get(id) {
    ...
    return publicTask(await refreshExternal(task,true),project);
  }
