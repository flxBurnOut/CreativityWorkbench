...
export function createTaskManager(repository, { env = process.env, fetchImpl = fetch, onError = () => {} } = {}) {
  ...
  async function submit(input) {
    ...
    return serial(async () => {
      ...
      const pending=unfinished.find(t => t.projectId === input.projectId && t.snapshot?.type === project.type && (t.kind === input.kind) && (t.args.objectId ?? '') === (input.args.objectId ?? ''));
  ...
  return { ready, submit, list, get, cancel, dismiss, stop, completeHandoff };
}
