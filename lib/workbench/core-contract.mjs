...
const patch = z.object({
  ...
  craftGoal:z.string().max(4000).optional(),
...
export const coreTools = {
  craft_generate:{description:'生成本地三维文创网格，输出 Blender/GLB。提供 textureOf=已有模型 taskId、textureMode=image 时，复用现有图片服务生成平面文化图案，本机贴到器皿外壁，不需要 HY 或 COS；支持碗盘盆瓶罐锅瓢壶。goal 沿用原资产 prompt，可选 texturePrompt（200字）。dispatch=conversation 在当前 WorkBuddy 对话回传原任务 PNG，完成用 task_complete_handoff，不用 craft_complete_plan。textureMode=hy 或省略模式保留旧腾讯云纹理路径，需要 COS。生图按已配置服务计费，须用户明确要求；不改器形、保留历史。普通建模可用 planJson 或文字服务/WorkBuddy 原任务交接。相同 requestId 仅查询原任务，不重复提交。',schema:z.object({...mutation,goal:z.string().trim().min(1).max(4000),planJson:z.string().min(2).max(16000).optional(),retryOf:id.optional(),textureOf:id.optional(),textureMode:z.enum(['image','hy']).optional(),dispatch:z.enum(['auto','conversation']).optional(),texturePrompt:z.string().trim().min(1).max(400).refine(v=>Array.from(v).length<=200).optional()}).strict(),openWorld:true},
  craft_complete_plan:{description:'接续已存在三维任务，只提交受限建模 JSON 方案，不新建任务或写任意脚本。taskId 使用网页原任务；从 task_get.handoffInstructions 获取字段规则。Runtime 验证后在同一任务启动 Blender，成功自动同步到网页。同方案可幂等重试，不接受替换已确定方案或复活取消任务。',schema:z.object({taskId:id,planJson:z.string().min(2).max(16000)}).strict()},
  ...
  project_create: { description: '保存新创意项目。requestId 用于幂等重试；craft 项目可用 craft_generate 生成受限器具与建筑构件的真实 Blender 资产。', schema: z.object({ requestId: id, idea: z.string().trim().min(1).max(100000), title: z.string().max(200).default(''), type: workType.default('undecided') }).strict() },
