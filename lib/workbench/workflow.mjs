...
export function sourceLabel(p,key) {
  ...
  return {idea:'原始创意',title:'项目名称',brief:'创意方案',culture:'文化背景',art:'美术规范',novel:'当前正文',delivery:'交付要求',websiteSource:'已采用网站源码',websiteRequest:'网站任务包',designPackage:'设计资料包',videoFinal:'视频成品',videoRatio:'视频画幅',videoSettings:'视频合成设置',transfer:'类型转入资料'}[key]||key;
}
...
export function inputKeys(p,kind,args={}) {
  ...
  const base=['idea','brief','culture'];const c=p.concepts.find(c=>c.id===args.objectId);const s=p.video?.shots.find(s=>s.id===args.objectId);
  if(kind==='creative')return ['idea','brief','culture'];
  ...
  if(kind==='video-shot'||kind==='video-frame')return [p.culture?'culture':p.brief?'brief':'idea','art','videoRatio',...(s?.contentKeys||[]).map(k=>'content:'+k),...relatedShotConcepts(p,s).map(c=>'concept:'+c.id),...(s?[(kind==='video-frame'?'frame:':'shot:')+s.id]:[]),...(kind==='video-frame'?imageInputIds(p,args,kind):s?.referenceAssetId?[s.referenceAssetId]:[]).map(id=>'image:'+id)];
...
export function recordChanges(before,after,meta={}) {
  ...
  for(const t of present) {
    ...
    const deps=t.key.startsWith('concept:')?['culture',...objectSourceKeys(after,t.value)]:keys;
