...
export const BRANCH_FIELDS = ['brief','art','references','concepts','delivery','requests','novel','novelReferenceIds','video','website','websiteRequest','websiteSource','websiteSourceCandidate','designPackage','transfer','transfers'];
...
export function sourceLabel(p,key) {
  ...
  return {idea:'原始创意',title:'项目名称',brief:'创意方案',knowledge:'已选文化资料',culture:'文化背景',art:'美术规范',novel:'当前正文',delivery:'交付要求',websiteSource:'已采用网站源码',websiteRequest:'网站任务包',designPackage:'设计资料包',videoFinal:'视频成品',videoRatio:'视频画幅',videoSettings:'视频合成设置',transfer:'类型转入资料',transferSources:'来源类型最新状态'}[key]||key;
}
...
export function inputKeys(p,kind,args={}) {
  ...
  if(kind==='website'||kind==='design-package')return ['title',...base,...content,'art','delivery',...(selectedTransfers(p).length?['transfer','transferSources']:[]),...(p.type==='website'&&p.websiteSource?['websiteSource']:[]),...p.concepts.filter(c=>c.savedAssetId).map(c=>'concept:'+c.id),...p.references.map(r=>'reference:'+r.id),...p.assets.filter(a=>(p.websiteRequest?.assetIds??p.concepts.map(c=>c.savedAssetId)).includes(a.id)||p.references.some(r=>r.assetId===a.id)).map(a=>'image:'+a.id)];
  return base;
}
