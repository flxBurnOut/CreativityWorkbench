...
export async function cleanupStorage(repository, { execute = false, confirmationToken, minimumAgeDays = 7 } = {}) {
  ...
  for(const task of tasks) {
    ...
    // A craft pattern completes with craftAsset, not result.asset. Its original
    // normalized PNG is the only possible durable proof for result.png; never
    // compare a resized atlas or discard a failed/cancelled job's source.
    const pattern=task.kind==='craft-model'&&task.args?.textureMode==='image'&&task.result?.craftAsset?task.patternImage:undefined;
    const result=website||task.result?.asset||task.result?.videoClip||task.result?.videoAudio||(pattern?.originalFileId?{fileId:pattern.originalFileId}:undefined);
    if(!result?.fileId)continue;
    const media=Boolean(task.result?.asset||pattern);
