...
import { workflowInputs, fingerprint, selectedTransfers } from './workflow.mjs';
...
export async function importWebsiteSource(bytes,repository,metadata={}) {
  validateWebsiteVerification(metadata);
  const inspected=repository.inspectWebsiteSource?repository.inspectWebsiteSource(bytes):inspectWebsiteZip(bytes);const stored=await repository.putOutput(bytes,'zip');
  return {...stored,verificationMethod:metadata.verificationMethod||'not-tested',verificationResult:metadata.verificationResult||'not-tested',verificationEvidence:metadata.verificationEvidence||'',entryCount:inspected.entryCount,...(inspected.previewPath?{previewPath:inspected.previewPath}:{}),description:metadata.description||'用户导入的网站源码',instructions:metadata.instructions||'',verification:metadata.verification||'未提供执行端验证记录；仅完成 ZIP 结构检查。',importedAt:Date.now(),...(metadata.taskId?{taskId:metadata.taskId}:{}),...(metadata.requestSource?{requestSource:metadata.requestSource}:{})};
}
export async function prepareDesignPackage(task,{repository}) {
  ...
  const brief={type:p.type,title:p.title,idea:p.idea,brief:p.brief,culture:p.culture,themeKnowledge:knowledgeText(p),inheritedDrafts:selectedTransfers(p),content:p.content[p.type],art:p.art,delivery:p.delivery,assets:roles,
  ...
  const files=[textEntry('DESIGN.json',JSON.stringify(brief,null,2)),textEntry('README.md',`# ${p.title}\n\n${brief.status}\n\n读取 DESIGN.json、SOURCES.json 与实际图片，按本次明确事项继续完善。${brief.pending}`),textEntry('SOURCES.json',JSON.stringify(inputs,null,2))];
  if(p.knowledge?.length)files.push(textEntry('KNOWLEDGE.md',knowledgeText(p)));
  for(const role of roles)files.push({name:role.filename,chunks:()=>repository.fileChunks(p.assets.find(a=>a.id===role.id).fileId,true)});
  const output=await repository.putOutputStream(streamZip(files));
