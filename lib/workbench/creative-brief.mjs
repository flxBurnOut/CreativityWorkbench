...
当前方案包含：一句话概念、作品类型、目标受众、核心表达、岭南文化关联、内容简述、必须保留的要点、明确排除的内容。缺少且不妨碍探索的信息可写待确定，不要阻断创作。
...
export async function generateCreativeBrief(input, { env = process.env, fetchImpl = fetch, signal } = {}) {
  const clean = validateCreativeInput(input);
  return completeCreativeBrief([
    { role: 'system', content: system },
    { role: 'user', content: JSON.stringify({ operation: actions[clean.action], workType: types[clean.type], originalIdea: clean.idea, currentBrief: clean.brief, cultureContext: clean.culture, modification: clean.instruction }) },
  ], { apiKey: env.DEEPSEEK_API_KEY, fetchImpl, signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(120000)]) : AbortSignal.timeout(120000) });
}
