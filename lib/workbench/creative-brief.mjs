import { completeCreativeBrief } from './adapters/deepseek.mjs';
import { textServiceConfig, textServiceStatus } from './text-config.mjs';
...
export function creativeStatus(env = process.env) {
  return { ...textServiceStatus(env), stage: 'creative', actions: Object.keys(actions) };
}
...
export async function generateCreativeBrief(input, { env = process.env, fetchImpl = fetch, signal, onPrompt } = {}) {
  ...
  return completeCreativeBrief(messages, { ...textServiceConfig(env), fetchImpl, signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(120000)]) : AbortSignal.timeout(120000) });
}
