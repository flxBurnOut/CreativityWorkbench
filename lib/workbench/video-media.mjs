import { runMediaProcess, boundedMediaArgs } from './media-process.mjs';
...
export function mediaCommand(binary, args, { signal, cwd } = {}) {
  return runMediaProcess(binary, boundedMediaArgs(args, args.includes('-show_streams')), { signal, cwd });
}
...
export async function probeMedia(path, { env = process.env, signal } = {}) {
  ...
  const [darW,darH]=(video?.display_aspect_ratio||'').split(':').map(Number);
  const rotation=Number(video?.side_data_list?.find(s=>s.rotation!==undefined)?.rotation||0);
  const ratio=darW>0&&darH>0?darW/darH:video?.width/video?.height;
  return { duration, width: video?.width, height: video?.height, ...(video?{displayAspectRatio:Math.abs(rotation)%180===90?1/ratio:ratio}:{}), videoCodec: video?.codec_name, audio: data.streams?.some(s => s.codec_type === 'audio') || false };
}
  ...
  return withMediaDirectory(repository, async dir => {
    ...
    for (let i = 0; i < v.shots.length; i++) {
      ...
      if (s.audio && s.narration.trim()) {
        ...
        if (v.keepAudio && clip.audio) { audioFilter=`[0:a]volume=0.35[original];[original][1:a]amix=inputs=2:duration=longest:normalize=0,alimiter=limit=0.95,apad=whole_dur=${s.duration}[mixed]`; audioMap='[mixed]'; }
      } else if (v.keepAudio && clip.audio) audioMap='0:a:0';
      else { inputs.push('-f','lavfi','-i',`anullsrc=r=48000:cl=stereo:d=${s.duration}`); audioMap='1:a:0'; }
      await encode([...inputs,...(audioFilter?['-filter_complex',audioFilter]:[]),'-map','0:v:0','-map',audioMap,'-vf',`scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2,setsar=1,fps=24,format=yuv420p`,...(audioFilter?[]:['-af',`apad=whole_dur=${s.duration}`]),'-t',String(s.duration),'-c:v','libx264','-preset','fast','-crf','20','-c:a','aac','-ar','48000','-ac','2',`part-${i}.mp4`]);
    }
