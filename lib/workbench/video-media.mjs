...
export async function composeVideo(task, repository, options) {
  ...
  for (const s of v.shots) {
    if (!s.clip || s.clip.source !== shotSource(s,v.ratio)) throw new ServiceError(400,'stale_clip',`镜头「${s.title}」缺少当前版本的视频，请生成或重新导入。`);
    if (s.narration.trim() && (!s.audio || s.audio.source !== audioSource(s))) throw new ServiceError(400,'missing_audio',`镜头「${s.title}」需要匹配当前旁白的配音文件，可通过 WorkBuddy 生成或导入。`);
  }
  return withMediaDirectory(repository, async dir => {
    ...
    for (let i = 0; i < v.shots.length; i++) {
      ...
      const inputs = ['-protocol_whitelist','file,pipe','-i',input]; let audioMap;
      ...
      await encode([...inputs,'-map','0:v:0','-map',audioMap,'-vf',`scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2,setsar=1,fps=24,format=yuv420p`,'-af','apad','-t',String(s.duration),'-c:v','libx264','-preset','fast','-crf','20','-c:a','aac','-ar','48000','-ac','2',`part-${i}.mp4`]);
    }
    ...
    return { videoFinal: { ...result, subtitleFileId: caption.fileId, duration, source: composeSource(v), taskId: task.id } };
  });
}
