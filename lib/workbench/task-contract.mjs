...
import { workflowInputs, conceptValue } from './workflow.mjs';
export const taskKinds = ['creative','content','art','objects','novel','image','cover','video-frame','video-plan','video-shot','video-audio','video-compose','website','website-build','design-package'];
...
export function taskSource(p, kind, args) {
  ...
  if (kind === 'content') return JSON.stringify([...common, p.content[p.type],p.transfer??null,args.fromNovel?p.novel:null, args.instruction ?? p.requests[1], args]);
  if (kind === 'novel') return JSON.stringify([p.id,p.type,workflowInputs(p,kind,args),p.requests[4],args]);
  if (kind === 'art') return JSON.stringify([...common, p.content[p.type], p.art, p.references, p.requests[2]]);
