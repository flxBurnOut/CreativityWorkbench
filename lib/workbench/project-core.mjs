...
import { activeArtPrompt, artFromPrompt } from './art-prompt.mjs';
...
export function applyTaskResult(project, task, selection = {}) {
  if (task.projectId !== project.id || task.status !== 'succeeded' || !task.result || !sameTaskSource(taskSource(project, task.kind, task.args),task.source)) throw new Error('生成依据已改变，未覆盖当前项目；你仍可复制或下载结果。');
  ...
  if (r.art) next.art = artFromPrompt(activeArtPrompt(r.art));
  if (r.objects) {
  ...
  if (r.craftAsset) next.craftAsset=r.craftAsset;
  if (r.asset) {
