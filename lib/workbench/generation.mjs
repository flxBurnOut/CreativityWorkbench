...
import { textServiceStatus } from './text-config.mjs';
...
import { taskKinds, contentKeys, taskSource, sameTaskSource } from './task-contract.mjs';
import { ServiceError } from './errors.mjs';
import { artFromPrompt } from './art-prompt.mjs';
...
import { selectedTransfers, workflowInputs, selectedNovelConcepts, conceptValue, dependencySnapshot, inputKeys, imageInputIds } from './workflow.mjs';
import { prepareDesignPackage } from './workflow-delivery.mjs';
import {validateCraftTask,runCraftTask} from './craft-runtime.mjs';
...
export function validateGeneration(input, project) {
  ...
  if (JSON.stringify(args).length > (kind==='craft-model'?30000:10000)) invalid('修改要求过长。');
  if (!sameTaskSource(taskSource(project, kind, args),input.source)) throw new ServiceError(409, 'conflict', '生成依据已改变，请等待保存完成并基于当前内容重新提交。');
  if (!project.idea.trim()) invalid('请先写下创意。');
  if(kind==='craft-model')validateCraftTask(input,project);
  ...
  if (kind === 'art' && args.instruction !== undefined && !text(args.instruction, 8000)) invalid('修改要求格式无效。');
  if (kind === 'content') {
...
export async function runGeneration(task, { repository, env, fetchImpl, signal, update }) {
  ...
  if(task.kind==='craft-model')return runCraftTask(task,{repository,env,fetchImpl,signal,update});
  ...
  if (task.kind === 'creative') return generateCreativeBrief({ action: task.args.action, type: p.type, idea: p.idea, brief: p.brief, culture: p.culture, themeKnowledge:knowledgeText(p), instruction: p.requests[0] }, { ...options, onPrompt });
  if (['image','cover','video-frame'].includes(task.kind)) {
    ...
    if (task.args.provider === 'workbuddy') {
      const handoff = await repository.handoff(task.id, prompt, images, 'png', {projectId:task.projectId,kind:task.kind,objectId:task.args.objectId});
      const {handoffMessage:message} = existingImageHandoff({...task,handoff});
  ...
  const context = { workType: p.type, idea: p.idea, brief: p.brief, culture: p.culture, themeKnowledge:knowledgeText(p), content: p.content[p.type], ...(task.kind === 'art' ? { art: artContext(p.art) } : {}), instruction: p.requests[task.kind === 'art' ? 2 : task.kind === 'objects' ? 3 : task.kind === 'novel' ? 4 : 1], args: task.args };
  if(['content','art'].includes(task.kind)&&task.args.instruction!==undefined)context.instruction=task.args.instruction;
  ...
  if(task.kind==='content'&&selectedTransfers(p).length)context.inheritedDraft=selectedTransfers(p);
  let spec;
  if (task.kind === 'content') {
    ...
    spec = '仅编写或按 instruction 修改一段可直接使用的美术风格提示词，不生成图片。只返回 {"fullPrompt":"完整美术风格提示词"}，不要另建分项字段。art.fullPrompt 是当前稿，用户明确要求只修改指定部分时保留其余文字与限制。方向、造型材质、色彩光线及保持排除项均用连贯文字表达，不强行补齐未要求的项目。旧记录如有重复可整理；存在影响方向的矛盾应提示待确定，不暗设字段优先级。配合当前作品类型：视频服务主体辨识与运动画面，网站服务视觉与信息层级，3D 概念服务形体与材质，小说服务人物环境想象。本次没有附图，不编造看图结论。';
  ...
  if (task.kind === 'art') {
    if (!text(value.fullPrompt) || !value.fullPrompt.trim()) badOutput();
    return { art: artFromPrompt(value.fullPrompt), model };
  }
...
export function generationStatus(env) { return { text: textServiceStatus(env), images: imageConfig(env), video: videoConfig(env), website:{scope:'网站生成提示词与素材交接；模型负责完整实现，保留旧网站文件', workbuddyConfigured:Boolean(env.WORKBUDDY_ACCESS_TOKEN?.trim())}, novel: { scope: '短篇正文，TXT / Markdown' } }; }
