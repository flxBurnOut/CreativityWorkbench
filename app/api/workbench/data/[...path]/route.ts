...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (!/^(source-preview\/[a-f0-9]{64}\.zip\/[^?#]+|core\/(project_get|video_frame_fit|task_complete_handoff)|workspace|status|settings|recovery|media|media\/[a-f0-9]{64}|files\/(website-source|mp4|wav|[a-f0-9]{64}\.(mp4|wav|html|zip|srt)|restore\/[a-f0-9]{64}\.(mp4|wav|html|zip|srt))|tasks|tasks\/[a-zA-Z0-9_-]{1,80}(\/(cancel|dismiss))?)$/.test(route)) return Response.json({ error: '接口不存在。' }, { status: 404 });
