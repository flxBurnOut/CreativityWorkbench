...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (!/^(workspace|status|media|media\/[a-f0-9]{64}|files\/(mp4|wav|[a-f0-9]{64}\.(mp4|wav|html|zip|srt)|restore\/[a-f0-9]{64}\.(mp4|wav|html|zip|srt))|tasks|tasks\/[a-zA-Z0-9_-]{1,80}(\/(cancel|dismiss))?)$/.test(route)) return Response.json({ error: '接口不存在。' }, { status: 404 });
  ...
  try {
    ...
    for(const key of ['Content-Length','Content-Range','Accept-Ranges','Content-Disposition','Content-Security-Policy','Referrer-Policy']){const value=response.headers.get(key);if(value)responseHeaders[key]=value;}
