...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (!/^(source-preview\/[a-f0-9]{64}\.zip\/[^?#]+|core\/(project_get|video_frame_fit|task_complete_handoff|image_select|task_adopt)|workspace|status|settings|recovery|media|media\/[a-f0-9]{64}|files\/(website-source|mp4|wav|[a-f0-9]{64}\.(mp4|wav|html|zip|srt)|restore\/[a-f0-9]{64}\.(mp4|wav|html|zip|srt))|tasks|tasks\/[a-zA-Z0-9_-]{1,80}(\/(cancel|dismiss))?)$/.test(route)) return Response.json({ error: '接口不存在。' }, { status: 404 });
  ...
  try {
    ...
    if (!['GET','HEAD'].includes(request.method)) {
      ...
      let size=0;const limit=(route.startsWith('files/')?128:20)*1024*1024;
    ...
    const options = { method: request.method, headers, body, duplex:'half', cache: 'no-store' as RequestCache, signal: AbortSignal.any([request.signal,AbortSignal.timeout(route.startsWith('files/')?240000:30000)]) };
