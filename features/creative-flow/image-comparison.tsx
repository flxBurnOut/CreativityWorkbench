...
export function ImageComparison({original,revised,onConfirm}:{original?:ImageAsset;revised:ImageAsset;onConfirm?:(review:ImageReview)=>void|Promise<void>}) {
  ...
  return <div className="image-comparison"><p>本次修改要求：{revised.source?.instruction||'按原有修改要求逐项核对。'}</p><p className="muted">文件不同不代表修改有效。先按整体预览尺寸检查，再同步放大到雨渍、边缘、纹理等具体位置。</p>
    ...
    {onConfirm&&<><label className="workflow-check"><input type="checkbox" checked={changes} onChange={e=>setChanges(e.target.checked)}/>指定修改已经清晰可辨</label><label className="workflow-check"><input type="checkbox" checked={preserved} onChange={e=>setPreserved(e.target.checked)}/>应保留的主体、构图和风格已核对</label><Field label="对比观察记录"><textarea rows={2} value={notes} onChange={e=>setNotes(e.target.value)} placeholder="例如：伞缘磨损清晰可见，伞柄与背景未改变。"/></Field><Button disabled={saving||!original||!changes||!preserved||!notes.trim()} onClick={async()=>{setSaving(true);try{await onConfirm({assetId:revised.id,parentAssetId:original!.id,changesVisible:true,preserved:true,notes:notes.trim(),checkedAt:Date.now()});}finally{setSaving(false);}}}>{saving?'正在选用…':'已核对，选用修改图'}</Button><p className="muted">若未达到要求，保留候选并继续修改；不勾选通过，也不自动重新生成。</p></>}
  </div>;
}
