...
const services = [
  ...
  { title: '图像生成', detail: '兼容 Images API 的外部服务', key: 'IMAGE_API_KEY', base: 'IMAGE_API_BASE_URL', model: 'IMAGE_API_MODEL' },
  { title: '视频生成', detail: 'Runway Gen-4.5', key: 'VIDEO_API_KEY', base: 'VIDEO_API_BASE_URL' },
...
export function ServiceSettings({ onSaved }: { onSaved: () => void }) {
  ...
  async function save() {
    ...
    try {
      ...
      setDraft({}); setCleared([]); setMessage('配置已保存，后续生成立即生效。'); onSaved();
  ...
  return <details className="settings-section api-settings"><summary>API 配置 <span>添加或更换密钥</span></summary>
    ...
    {!data ? <Button variant="secondary" onClick={() => void load()}>重新读取配置</Button> : <form onSubmit={event => { event.preventDefault(); void save(); }}>
      <fieldset disabled={busy} className="api-settings-fields">
        ...
        <p className="settings-note">页面保存的配置优先于环境文件。清除密钥会停用对应授权；图像和视频请在生成时选择外部服务。</p>
