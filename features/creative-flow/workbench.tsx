...
export default function Workbench() {
  ...
  useEffect(() => { document.title = project ? `${project.title} · 粤象·岭南文化创意工作台` : '粤象·岭南文化创意工作台'; }, [project?.title]);
  ...
  return <div className={`workbench ${project ? 'workbench-project' : ''}`}><a className="skip-link" href="#main-content" onClick={event => { event.preventDefault(); document.getElementById('main-content')?.focus(); }}>跳到主要内容</a>
    ...
    {!store.ready ? <main id="main-content" tabIndex={-1} className="loading-page"><span className="loading-leaf"><Icon name="leaf" size={32} /></span><h1>{store.error ? '你的原有草稿保持不变' : '推开门，等灵感进来'}</h1><p>{store.error ? '可以重试读取，或进入不保存的临时体验。' : '正在读取本机服务的创作记录…'}</p></main> : !project ? <main id="main-content" tabIndex={-1} className="home-main">
      <section className="welcome-scene"><div className="welcome-copy"><p className="eyebrow"><span />岭南文化 · 创作之间</p><h1>把岭南创意，<br />做成一件作品<span className="title-dot">。</span></h1><p className="welcome-description">从故事、影像到网站与文创，让一个想法慢慢成形。</p><div className="welcome-actions"><Button icon="plus" onClick={() => setDialog({ kind: 'new' })}>开始创作</Button><Button variant="ghost" onClick={() => setDialog({ kind: 'demo' })}>先看示例<Icon name="arrow" size={16} /></Button></div><span className="scene-caption">骑楼光影　·　广彩花色　·　葵扇清风</span></div><div className="welcome-art"><img src="/art/courtyard.svg" alt="岭南骑楼与院落旁，葵扇和彩瓷静置在手作台上的原创插画" /><span className="art-note">一方庭院，容得下许多想象。</span></div></section>
      <CapabilityCards onCreate={type => setDialog({ kind: 'new', type })}/><section className="home-process" aria-labelledby="home-process-title"><div className="home-process-heading"><h2 id="home-process-title">从想法到作品，接下来这样做</h2><Button variant="ghost" onClick={() => setDialog({kind:'help'})}>查看操作说明<Icon name="arrow" size={15}/></Button></div><ol><li><span className="home-step-number">01</span><div><h3>写下目标</h3><p>告诉工作台，你想做什么。</p></div></li><li><span className="home-step-number">02</span><div><h3>查看初稿</h3><p>围绕实际结果，提出修改想法。</p></div></li><li><span className="home-step-number">03</span><div><h3>保存与交付</h3><p>选定满意的版本，下载作品。</p></div></li></ol></section>
