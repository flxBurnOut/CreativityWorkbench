...
export default function Workbench() {
  ...
  const stageProps = project ? { project, edit, notice, unavailable, demo: isDemo, demoState } : null;
  ...
  return <div className={`workbench ${project ? 'workbench-project' : ''}`}><a className="skip-link" href="#main-content" onClick={event => { event.preventDefault(); document.getElementById('main-content')?.focus(); }}>跳到主要内容</a>
    ...
    {!store.ready ? <main id="main-content" tabIndex={-1} className="loading-page"><span className="loading-leaf"><Icon name="leaf" size={32} /></span><h1>{store.error ? '你的原有草稿保持不变' : '推开门，等灵感进来'}</h1><p>{store.error ? '可以重试读取，或进入不保存的临时体验。' : '正在读取此浏览器的创作记录…'}</p></main> : !project ? <main id="main-content" tabIndex={-1} className="home-main">
      ...
      <div key={project.id} className="stage-body">{stageProps && <>{project.stage === 0 && <CreativeStage {...stageProps} />}{project.stage === 1 && <ContentStage {...stageProps} />}{project.stage === 2 && <ArtStage {...stageProps} />}{project.stage === 3 && <ConceptsStage {...stageProps} />}{project.stage === 4 && <FinalStage {...stageProps} />}<RequestComposer project={project} edit={edit} unavailable={unavailable} goToStage={stage => { edit(p => ({ ...p, stage })); window.scrollTo(0, 0); }} /></>}</div><footer className="project-footer"><Icon name="leaf" size={14} />让作品慢慢成形，也让每一步都留得住。</footer>
    </main>}
    ...
    {dialog?.kind === 'unavailable' && <Modal title={`${dialog.action}尚未接入`} subtitle="当前版本提供完整的前端流程，生成服务将在后续接入。" onClose={() => setDialog(null)}><div className="service-placeholder"><span className="empty-symbol"><Icon name="spark" size={28} /></span><p>你可以继续编辑文字、整理参考图片和暂存项目。此操作没有发起生成请求，也没有产生费用。</p></div><div className="modal-actions"><Button variant="secondary" onClick={() => setDialog({ kind: 'settings' })}>查看服务状态</Button><Button onClick={() => setDialog(null)}>继续创作</Button></div></Modal>}
    {dialog?.kind === 'settings' && <Modal title="工作台设置" subtitle="把技术细节留在这里，让创作过程保持简单。" onClose={() => setDialog(null)}><div className="settings-section"><h3>生成服务</h3>{[{ icon: 'book' as const, name: '文字与美术提示词', text: '创意完善、内容生成、提示词整理' }, { icon: 'image' as const, name: '概念图与图片返工', text: '生成参考图、基于图片进行修改' }, { icon: 'gift' as const, name: '成品生成', text: '小说、视频、文创与网站交付' }].map(item => <div className="service-row" key={item.name}><span className="service-icon"><Icon name={item.icon} /></span><div><strong>{item.name}</strong><p>{item.text}</p></div><span className="tag">未接入</span></div>)}<p className="settings-note">当前没有 API 配置功能，也不收集或保存密钥。</p></div><div className="settings-section"><h3>草稿与图片</h3><div className="local-storage-note"><Icon name="check" /><div><strong>{store.status === 'temporary' ? '当前为临时体验' : '暂存在当前浏览器'}</strong><p>同一浏览器、同一地址下可以恢复项目。清理网站数据或更换浏览器后，草稿不会自动迁移；服务端保存将在后续接入。</p></div></div></div><div className="modal-actions"><Button onClick={() => setDialog(null)}>返回工作台</Button></div></Modal>}
    {dialog?.kind === 'help' && <Modal title="从想法，到作品" subtitle="只需要说出想法、提出修改、留下满意的结果。" onClose={() => setDialog(null)}><ol className="help-steps">{STAGES.map((stage, index) => <li key={stage}><span>{index + 1}</span><div><h3>{stage}</h3><p>{['把一句想法展开成清楚的创意，建立与岭南文化的联系。', '整理故事、脚本、图案或网页内容，可以直接粘贴已有文稿。', '用文字确定色彩、造型和材质，参考图片按需添加。', '为角色、地图和物件整理图片；点击保存后进入选定参考集。', '检查生成依据与交付要求；接入服务后预览、下载完整作品。'][index]}</p></div></li>)}</ol><div className="help-note">随时切换阶段，纯文字作品可以跳过美术和概念图。当前 AI 尚未接入，文字与本地图片操作可以直接使用。</div><div className="modal-actions"><Button variant="secondary" onClick={() => setDialog({ kind: 'demo' })}>先体验一个示例</Button><Button onClick={() => setDialog(null)}>开始创作</Button></div></Modal>}
