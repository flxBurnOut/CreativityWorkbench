...
export function useGeneration(project:Project|null,demo:boolean,edit:EditProject,notice:Notice,flush:()=>Promise<void>) {
  ...
  async function adopt(task:GenerationTask,useSuggestedTitle=false,advance=false){
    ...
    try{
      ...
      notice(task.result?.websiteRequest?'已保存网站生成任务包，待执行模型实现网站':task.kind==='video-frame'?'首帧候选已保存，核对后选择“采用此首帧”':task.result?.asset&&task.kind!=='cover'?'已加入候选图；满意后点击保存，加入选定参考。':'已采用生成结果',()=>{
  ...
  const visible=tasks.filter(t=>(!contentMode||matchesContent(t))&&(showHistory||!t.dismissed)).slice(0,showHistory?100:50);
  const panel=!demo&&project&&<section className="generation-tasks" aria-label="生成任务与结果">
    {contentMode&&<div className="section-heading"><div><h3>修改建议与检查结果</h3><p>{visible.length?'比较后采用，当前稿才会更新。':'生成的建议会出现在这里，当前稿始终可以直接编辑。'}</p></div></div>}
    ...
    {visible.map(task=>{
      ...
      return <article className="surface task-card" key={task.id}><div className="section-heading"><div><h3>{names[task.kind]}{task.targetName ? ` · ${task.targetName}` : ""} · {task.kind==='website'&&task.status==='succeeded'?'任务包已准备':statuses[task.status]}</h3><p>{new Date(task.createdAt).toLocaleString('zh-CN')}{task.dismissed?' · 已收起／采用':''}</p></div>{!['succeeded','failed','cancelled'].includes(task.status)&&<Button variant="ghost" onClick={async()=>{try{await api('tasks/'+task.id+'/cancel','POST',{});await refresh();}catch(e){setError(String(e));}}}>取消等待</Button>}</div>
        ...
        {task.handoffMessage&&(task.kind==='website'||['waiting_external','uncertain','failed'].includes(task.status))&&<details><summary>WorkBuddy 请求与交接</summary><textarea readOnly aria-label="WorkBuddy 交接请求" rows={5} value={task.handoffMessage}/><Button variant="secondary" onClick={()=>void navigator.clipboard.writeText(task.handoffMessage!).then(()=>notice('已复制 WorkBuddy 请求')).catch(()=>notice('复制失败，请选中文字手动复制。'))}>复制请求</Button></details>}
        ...
        {task.status==='succeeded'&&!task.dismissed&&!r?.notes&&<>{stale&&<p className="muted">生成依据已更新，结果保留供复制或下载，不能覆盖当前内容。</p>}<Button disabled={stale||chosen[task.id]?.sectionKeys?.length===0} onClick={()=>void adopt(task)}>{contentMode?(r?.sections?`采用选中的 ${chosen[task.id]?.sectionKeys?.length ?? Object.keys(r.sections).length} 个小节`:'采用此处修改'):r?.asset?(task.kind==='cover'?'采用封面':'加入候选图'):'采用结果'}</Button></>}
  ...
  return {controls:{run,busy:submitting,contentBusy,provider,ratio,setProvider,setRatio},panel};
}
