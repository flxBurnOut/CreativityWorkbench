...
export type GenerationControls = {
  error?:string;textAvailable?:boolean;videoProvider?:string;
  tasks?:GenerationTask[];refresh?:(force?:boolean)=>Promise<void>;
...
export function useGeneration(project:Project|null,demo:boolean,edit:EditProject,notice:Notice,flush:()=>Promise<void>,synchronize?:(force?:boolean)=>Promise<void>) {
  ...
  async function run(kind:string,args:Record<string,unknown>={},retryOf?:string) {
    ...
    try {
      ...
      const options=['image','cover','video-frame'].includes(kind)?{provider,ratio,...args}:['content','art'].includes(kind)?{instruction:p.requests[kind==='art'?2:1],...args}:args;
  ...
  const panel=!demo&&project&&<section className="generation-tasks" aria-label="生成任务与结果">
    <div className="inline-actions"><Button variant="ghost" onClick={()=>void refresh(true)}>{artMode?'刷新候选状态':'刷新任务与图片'}</Button><span className="muted">{artMode?'采用候选后更新当前提示词，已有图片保持原样。':'新结果会显示到对应对象；选用后才替换原图。'}</span></div>
    ...
    {visible.map(task=>{
      ...
      return <article className="surface task-card" id={'generation-task-'+task.id} key={task.id}><div className="section-heading"><div><h3>{names[task.kind]}{task.targetName ? ` · ${task.targetName}` : ""} · {task.kind==='website'&&task.status==='succeeded'?(task.args.guided?'网站初稿已收到':'任务包已准备'):r?.videoClip?.validation?.status==='failed'?'文件已收到 · 规格未通过':statuses[task.status]}</h3><p>{new Date(task.createdAt).toLocaleString('zh-CN')}{task.supersededBy?' · 历史请求，后续任务 '+task.supersededBy:task.dismissed?' · 已收起':''}{imageState?' · '+imageState.bindingLabel:''}</p></div>{!task.supersededBy&&!['succeeded','failed','cancelled','superseded'].includes(task.status)&&<Button variant="ghost" onClick={async()=>{try{await api('tasks/'+task.id+'/cancel','POST',{});await refresh();}catch(e){setError(String(e));}}}>取消等待</Button>}</div>
        ...
        {contentMode&&r?.sections?<div className="content-comparisons">{Object.entries(r.sections).map(([key,text])=>{const selected=chosen[task.id]?.sectionKeys??Object.keys(r.sections!);return <section key={key} className="content-comparison">{!task.dismissed&&<label className="workflow-check"><input type="checkbox" checked={selected.includes(key)} onChange={e=>setChosen(state=>({...state,[task.id]:{...state[task.id],sectionKeys:e.target.checked?[...selected,key]:selected.filter(k=>k!==key)}}))}/>采用「{sectionNames[key]||key}」</label>}<details open={Object.keys(r.sections!).length===1}><summary>{sectionNames[key]||key} · {text===project.content[project.type][key]?'内容未变':'查看对照'}</summary><div className="content-compare-grid"><Field label="当前稿"><textarea readOnly rows={8} value={project.content[project.type][key]||''}/></Field><Field label="修改建议"><textarea readOnly rows={8} value={text}/></Field></div></details></section>;})}</div>:contentMode&&r?.replacement!==undefined?<div className="content-compare-grid"><Field label="原选中文字"><textarea readOnly rows={6} value={String(task.args.selectedText||'')}/></Field><Field label="建议替换为"><textarea readOnly rows={6} value={r.replacement}/></Field></div>:artMode&&r?.art?<div className="content-compare-grid"><Field label="当前生效提示词"><textarea readOnly rows={8} value={activeArtPrompt(project.art)}/></Field><Field label="候选提示词"><textarea readOnly rows={8} value={activeArtPrompt(r.art)}/></Field></div>:preview&&<Field label={r?.notes?'检查建议':'结果预览'}><textarea readOnly rows={7} value={preview}/></Field>}
        {r?.culture&&<p className="creative-text">文化语境：{r.culture}</p>}
        {r?.title&&<p>建议名称：{r.title}</p>}
        {r?.art&&<p className="muted">这是一段文字提示词候选。采用后用于后续新图，本次未生成图片。</p>}
        ...
        {task.status==='succeeded'&&!task.dismissed&&!task.args.fromNovel&&!r?.notes&&!contentMode&&task.kind==='creative'&&project.stage<4&&<Button variant="secondary" disabled={stale} onClick={()=>void adopt(task,false,true)}>采用并进入下一阶段</Button>}
