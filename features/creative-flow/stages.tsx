...
import {imageOriginal} from '@/lib/workbench/image-results.mjs';
...
import { activeArtPrompt } from '@/lib/workbench/art-prompt.mjs';
...
export { ArtEditor as ArtStage } from './art-editor';

function StyleReferences({ project, edit, notice }: Pick<StageProps, 'project' | 'edit' | 'notice'>) {
  const [preview, setPreview] = useState<ImageAsset | null>(null);
  return <details className="surface style-image-tools"><summary>风格参考图片（可选） · {project.references.length} 张</summary><p className="muted">实际图片随后续出图请求发送。注明用途，例如只参考配色，不复制人物。</p><section className="surface"><div className="section-heading"><div><h3>参考图片 <span className="muted optional">可选</span></h3><p>说清楚想参考什么，图片不会替你决定整个风格。</p></div><UploadButton notice={notice} onUpload={asset => edit(p => ({ ...p, assets: [...p.assets, asset], references: [...p.references, { id: crypto.randomUUID(), assetId: asset.id, purpose: '' }] }))} /></div>{project.references.length ? <div className="style-reference-list">{project.references.map(reference => <div className="style-reference" key={reference.id}><button type="button" className="image-button" aria-label="放大参考图片" onClick={() => setPreview(project.assets.find(asset => asset.id === reference.assetId) ?? null)}><AssetImage asset={project.assets.find(asset => asset.id === reference.assetId)} alt="上传的美术参考" /></button><Field label="这张图只参考什么？"><input value={reference.purpose} placeholder="例如：只参考配色，不复制人物与背景" onChange={event => edit(p => ({ ...p, references: p.references.map(ref => ref.id === reference.id ? { ...ref, purpose: event.target.value } : ref) }))} /></Field><button type="button" className="icon-button" aria-label="移除参考图片" onClick={() => {
    ...
    }}><Icon name="close" /></button></div>)}</div> : <div className="upload-hint"><Icon name="image" size={25} /><span>没有图片也能继续。支持 PNG、JPG、WebP，单张不超过 8 MB。</span></div>}</section>{preview && <ImagePreview asset={preview} onClose={() => setPreview(null)} />}</details>;
}
...
export function ConceptsStage({ project, edit, notice, generation, demo, demoState }: StageProps) {
  const [comparison,setComparison]=useState<{conceptId:string;asset:ImageAsset;taskId?:string;original?:ImageAsset}|null>(null);
  ...
  async function saveConcept(concept: Concept,review?:Concept['imageReview'],taskId?:string,assetId?:string) {
    const selected=assetId||concept.candidateAssetId;if(!selected)return false;
    if(!demo&&generation?.selectImage)return generation.selectImage(concept.id,taskId?{taskId}:{assetId:selected},review);
    if (concept.candidateAssetId === concept.savedAssetId) return false;
  ...
  return <div className="stage-stack"><StyleReferences project={project} edit={edit} notice={notice}/><div className="surface generation-options"><Field label="图片生成方式"><select value={generation?.provider} onChange={e=>generation?.setProvider(e.target.value)}><option value="workbuddy">WorkBuddy</option><option value="external">外部 API</option></select></Field><Field label="图片比例"><select value={generation?.ratio} onChange={e=>generation?.setRatio(e.target.value)}><option value="1:1">方图</option><option value="3:2">横图</option><option value="2:3">竖图</option></select></Field><Button variant="secondary" disabled={generation?.busy} onClick={()=>generation?.run('objects')}>{concepts.length?'检查并更新对象设定':'从内容提取对象'}</Button></div>{demo && <DemoFeedback state={demoState} />}<section className="surface"><div className="section-heading"><div><h3>人物、场景与物件图片</h3><p>{project.type==='website'?'按页面内容准备需要的图像素材，每张图注明实际用途。':project.type==='craft'?'按主体、局部与材质需要准备概念参考，延续已采用的设计。':'只整理需要的人物、场景与物件。每张图与对象设定保持关联。'}</p></div><Button icon="plus" variant="secondary" onClick={() => setObjectForm({ name: '', description: '', category: category === 'all' ? 'object' : category })}>添加对象</Button></div><div className="category-tabs" aria-label="概念图类别">{(['all', 'character', 'map', 'object'] as const).map(key => <button type="button" key={key} aria-pressed={category === key} onClick={() => setCategory(key)}>{key === 'all' ? '全部' : CATEGORIES[key]}<span>{key === 'all' ? concepts.length : concepts.filter(c => c.category === key).length}</span></button>)}</div>
    {shown.length ? <div className="concept-grid">{shown.map(concept => {
      const incoming=generation?.imageResults?.[concept.id];
      const assetId = concept.candidateAssetId ?? concept.savedAssetId;
      const asset = incoming?.asset||project.assets.find(image => image.id === assetId);
      const isSaved = !incoming&&Boolean(concept.savedAssetId && (!concept.candidateAssetId || concept.savedAssetId === concept.candidateAssetId));
      return <article className="concept-card" key={concept.id}><div className="concept-image">{asset ? <button type="button" className="image-button" aria-label={`放大${concept.name}`} onClick={() => setPreview(asset)}><AssetImage asset={asset} alt={concept.name} /></button> : <div className="concept-placeholder"><Icon name={concept.category === 'object' ? 'gift' : concept.category === 'map' ? 'globe' : 'image'} size={33} /><span>等待第一张参考图</span><Button variant="ghost" icon="spark" disabled={generation?.busy} onClick={() => generation?.run('image',{action:'generate',objectId:concept.id})}>生成参考图</Button></div>}<span className={`image-status ${isSaved ? 'is-saved' : ''}`}>{incoming?.binding==='unbound'?'新结果待选用':isSaved ? '已选用图片' : asset ? '待选候选' : CATEGORIES[concept.category]}</span></div><div className="concept-info"><div className="section-heading"><h4>{concept.name}</h4><button type="button" className="icon-button" aria-label={`编辑${concept.name}`} onClick={() => setObjectForm({ id: concept.id, name: concept.name, description: concept.description, category: concept.category })}><Icon name="edit" size={16} /></button></div><p>{concept.description || '还没有对象描述'}</p><ObjectBindings project={project} concept={concept} edit={edit}/>{incoming&&<p className="muted" role="status">{incoming.stale?'这张结果的生成依据已有变化，先核对要求；原图保留。':'新结果已收到，可直接对比并选用；当前选用图仍保留。'}</p>}{asset ? <div className="concept-actions"><Button variant="secondary" icon="check" disabled={isSaved||generation?.busy||incoming?.stale} onClick={() => asset.source?.parentAssetId||incoming?.task.args.action==='edit'?setComparison({conceptId:concept.id,asset,taskId:incoming?.task.id,original:incoming?imageOriginal(project,incoming.task):undefined}):void saveConcept(concept,undefined,incoming?.task.id,asset.id)}>{isSaved ? '已选用' : '选用此图'}</Button><Button variant="ghost" disabled={generation?.busy||incoming?.stale} onClick={async() => {if(incoming?.binding==='unbound'&&!(await generation?.prepareImage?.(concept.id,incoming.task.id)))return;setNewStyle(false);setReworkId(concept.id);}}>修改图片</Button>{(asset.source?.parentAssetId||incoming?.task.args.action==='edit')&&<Button variant="ghost" onClick={()=>setComparison({conceptId:concept.id,asset,taskId:incoming?.task.id,original:incoming?imageOriginal(project,incoming.task):undefined})}>与原图对比</Button>}<Button variant="ghost" disabled={generation?.busy} onClick={()=>generation?.run('image',{action:'generate',objectId:concept.id})}>另作候选</Button><button type="button" className="icon-button subtle-danger" aria-label={incoming?.binding==='unbound'?`收起${concept.name}新结果`:`删除${concept.name}图片`} onClick={() => incoming?.binding==='unbound'?void generation?.dismissResult?.(incoming.task.id):deleteImage(concept)}><Icon name="trash" size={16} /></button></div> : <UploadButton children="上传已有图片" notice={notice} secondary={false} onUpload={image => uploadCandidate(concept.id, image)} />}</div></article>;
    ...
    {objectForm && <Modal title={objectForm.id ? '编辑对象' : '添加一个对象'} subtitle="只需要名称和一句描述，具体细节可以以后补充。" onClose={() => setObjectForm(null)}><form onSubmit={event => {
      ...
      edit(p => ({ ...p, concepts: form.id ? p.concepts.map(concept => concept.id === form.id ? { ...concept, name: form.name.trim(), category: form.category, description: form.description } : concept) : [...p.concepts, { id: crypto.randomUUID(), name: form.name.trim(), category: form.category, description: form.description, prompt: activeArtPrompt(p.art), revisionRequest: '' }] })); setObjectForm(null);
    ...
    {comparison&&<Modal wide title="原图与返工效果对比" onClose={()=>setComparison(null)}><ImageComparison key={comparison.asset.id} original={comparison.original||project.assets.find(a=>a.id===comparison.asset.source?.parentAssetId)} revised={comparison.asset} onConfirm={project.concepts.find(c=>c.id===comparison.conceptId)?.savedAssetId===comparison.asset.id?undefined:async review=>{
      const c=project.concepts.find(c=>c.id===comparison.conceptId);
      if(!c||(comparison.taskId?generation?.imageResults?.[c.id]?.task.id!==comparison.taskId:c.candidateAssetId!==comparison.asset.id)){notice('候选已改变，请关闭后重新核对当前图片。');return;}
      if(await saveConcept(c,review,comparison.taskId,comparison.asset.id))setComparison(null);
    }}/></Modal>}
...
export function FinalStage({ project, edit, demo, demoState, notice, generation }: StageProps) {
  ...
  return <div className="stage-stack"><section className="surface final-brief"><div className="section-heading"><div><h3>把准备好的灵感，交给下一步</h3><p>这里将汇集你的内容、视觉方向和选定参考。</p></div><span className="tag tag-soft">{TYPE_LABELS[project.type]}</span></div><div className="brief-summary"><div><span>当前创意</span><strong>{project.title}</strong></div><div><span>内容方案</span><strong>{contentCount ? `${contentCount} 个小节已有内容` : '尚未填写'}</strong></div><div><span>选定参考</span><strong>{savedCount} 张图片</strong></div></div><details className="prompt-details"><summary>查看生成依据摘要</summary><dl className="summary-list"><dt>想法</dt><dd>{project.idea || '尚未填写'}</dd><dt>岭南文化语境</dt><dd>{project.culture || '尚未补充，后续生成前需要形成具体语境。'}</dd><dt>视觉方向</dt><dd>{activeArtPrompt(project.art) || '尚未填写'}</dd></dl></details><div className="delivery-fields">{project.type === 'novel' && <Field label="文本交付格式"><select value={project.delivery.textFormat} onChange={event => edit(p => ({ ...p, delivery: { ...p.delivery, textFormat: event.target.value } }))}><option value="md">Markdown · 可编辑文本</option><option value="txt">TXT · 纯文本</option><option value="docx">Word · 待接入</option></select></Field>}{project.type === 'video' && <><Field label="画幅"><select value={project.delivery.ratio} onChange={event => edit(p => ({ ...p, delivery: { ...p.delivery, ratio: event.target.value } }))}><option>16:9</option><option>9:16</option><option>1:1</option></select></Field><Field label="目标时长"><input value={project.delivery.duration} placeholder="例如：约 60 秒，实际能力待接入验证" onChange={event => edit(p => ({ ...p, delivery: { ...p.delivery, duration: event.target.value } }))} /></Field></>}{project.type === 'craft' && <div className="delivery-note"><Icon name="gift" /><span>3D 文创资产列为最后阶段；当前可整理文化依据与概念参考，尚未接入模型生成。</span></div>}{project.type === 'website' && <div className="delivery-note"><Icon name="globe" /><span>计划交付网站预览、源码与运行说明，实际交互需逐项验证。</span></div>}{project.type === 'undecided' && <div className="delivery-note"><Icon name="leaf" /><span>可在项目名称旁选择作品类型，再规划对应交付。</span></div>}</div><NovelReferences project={project} edit={edit}/><InputSummary project={project} edit={edit} kind="novel"/><Field label="补充交付要求"><textarea rows={2} value={project.delivery.notes} placeholder="写下你希望保留的文字、文件要求或其他限制……" onChange={event => edit(p => ({ ...p, delivery: { ...p.delivery, notes: event.target.value } }))} /></Field><div className="generation-row"><AiNote children="此处为界面示例；请新建项目使用真实生成入口" /><Button disabled icon="spark">生成完整作品</Button></div></section>
...
export function RequestComposer({ project, edit, goToStage, generation }: Pick<StageProps, 'project' | 'edit' | 'unavailable' | 'creative' | 'generation'> & { goToStage: (stage: number) => void }) {
  const canRevise=project.stage===4&&project.type==='novel'&&project.novel;
  return <>{canRevise&&<div className="request-composer"><span className="composer-symbol"><Icon name="spark" size={22}/></span><label className="composer-field"><span>正文修改要求</span><textarea rows={2} value={project.requests[project.stage]} placeholder="说清需要改变的部分，其余内容保持。" onChange={event=>edit(p=>({...p,requests:p.requests.map((value,index)=>index===p.stage?event.target.value:value)}))}/><span className="muted">生成修改建议，采用前保留当前稿。</span></label><Button variant="secondary" disabled={generation?.busy||!project.requests[project.stage].trim()} onClick={()=>generation?.run('novel',{action:'revise'})}>按要求修改正文</Button></div>}<footer className="flow-footer"><div>{project.stage > 0 && <Button variant="ghost" icon="back" onClick={() => goToStage(project.stage === 4 && project.type === 'novel' ? 1 : project.stage - 1)}>返回{FLOW_LABELS[project.stage === 4 && project.type === 'novel' ? 1 : project.stage - 1]}</Button>}{project.type === 'novel' && project.stage === 1 && <Button variant="ghost" onClick={() => goToStage(2)}>我也需要配图</Button>}</div>{project.stage < 4 && <div className="flow-next"><span>{optionalStage(project.type,project.stage) ? '这一步可选，没有图片也能继续。' : '当前稿自动保存；生成建议请先预览并采用。'}</span><Button onClick={() => goToStage(nextStage(project))}>{project.type === 'novel' && project.stage === 1 ? '下一步：创作正文' : '下一步：'+FLOW_LABELS[nextStage(project)]}<Icon name="arrow" /></Button></div>}</footer></>;
}
