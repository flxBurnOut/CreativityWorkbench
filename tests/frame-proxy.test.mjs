...
test('frame and handoff UI reach only their required core routes and keep cross-origin protection',async t=>{
  ...
  for(const tool of ['project_get','video_frame_fit','task_complete_handoff'])assert.equal((await invoke(tool)).status,200);
  ...
  assert.equal((await invoke('task_complete_handoff','https://unrelated.example')).status,403);
  assert.equal((await invoke('workbench_call_json')).status,404);assert.equal(calls.length,3);
});
