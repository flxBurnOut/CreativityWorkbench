...
async function until(manager,id,status='succeeded') {for(let i=0;i<600;i++){const task=await manager.get(id);if(task.status===status)return task;if(task.status==='failed'&&status!=='failed')throw new Error(task.error);await new Promise(r=>setTimeout(r,30));}throw new Error('timeout');}
let fixture;
...
test('real FFmpeg imports, decodes, subtitles and composes multiple clips with audio',async()=>{
  ...
  for(const s of p.video.shots)s.clip={...clip,source:shotSource(s,p.video.ratio)};
  p.video.shots[0].audio={...audio,source:audioSource(p.video.shots[0])};p.video.music=audio;
  ...
  const srt=(await repo.output(result.videoFinal.subtitleFileId)).toString();assert.match(srt,/00:00:02,000 --> 00:00:04,000/);assert.equal(result.videoFinal.source,composeSource(p.video));
...
test('composition refuses missing narration and audio longer than the shot',async()=>{
  const {repo,p,dir}=await setup();const files=await clips();const clip=await importMedia(files.mp4,'mp4',repo,{env:{}});const s=p.video.shots[0];s.clip={...clip,source:shotSource(s,p.video.ratio)};s.narration='不能被省略的旁白';
...
test('sibling shot results remain adoptable, changed inputs reject stale results',async()=>{
  ...
  const file={fileId:'a'.repeat(64)+'.mp4',duration:2,source:shotSource(p.video.shots[0],p.video.ratio)};
...
test('website generation and manual rebuild use saved inputs without executing model code',async()=>{
  const {repo,p}=await setup('website');let calls=0;const manager=createTaskManager(repo,{env:{DEEPSEEK_API_KEY:'test'},fetchImpl:async(_url,options)=>{calls++;assert.match(options.body,/网站/);return Response.json({choices:[{finish_reason:'stop',message:{content:JSON.stringify(spec())}}]});}});
  const input=request(p,'website',{action:'generate'});await manager.submit(input);const task=await until(manager,input.id);const adopted=applyTaskResult(p,task);adopted.website.spec=structuredClone(adopted.website.spec);adopted.website.spec.title='改名后的网站';await repo.saveWorkspace(ws(adopted),1,randomUUID());
  const rebuild=request(adopted,'website-build');await manager.submit(rebuild);const built=await until(manager,rebuild.id);assert.equal(calls,1);assert.match((await repo.output(built.result.website.previewFileId)).toString(),/改名后的网站/);assert.notEqual(built.result.website.previewFileId,task.result.website.previewFileId);manager.stop();
});
