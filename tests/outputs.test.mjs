...
import { createTestDirectory, trackTestManager, closeTestServer } from './helpers/test-directory.mjs';
...
import { createTaskManager as makeTaskManager } from '../lib/workbench/tasks.mjs';
const createTaskManager = (repo, options) => trackTestManager(repo, makeTaskManager(repo, options));
...
async function setup(t,type='video'){const scope=await createTestDirectory(t,'creative-output');const dir=scope.directory;const repo=createRepository(dir);const p=createProject('岭南日常',type,'本地验证');if(type==='video')p.video=video();await repo.saveWorkspace(ws(p),0,randomUUID());return {repo,p,dir,scope};}
...
let fixture;
async function clips(){return fixture??=(async()=>{const scope=await createTestDirectory(null,'creative-clips');const dir=scope.directory;try{await mediaCommand(ffmpeg,['-nostdin','-v','error','-f','lavfi','-i','testsrc2=size=320x180:rate=24','-f','lavfi','-i','sine=frequency=440:sample_rate=48000','-t','2','-c:v','libx264','-pix_fmt','yuv420p','-c:a','aac',join(dir,'test.mp4')]);await mediaCommand(ffmpeg,['-nostdin','-v','error','-f','lavfi','-i','sine=frequency=600:sample_rate=48000','-t','1','-ac','2',join(dir,'test.wav')]);return {mp4:await readFile(join(dir,'test.mp4')),wav:await readFile(join(dir,'test.wav'))};}finally{await scope.dispose();}})();}

test('real FFmpeg imports, decodes, subtitles and composes multiple clips with audio',async t =>{
  const {repo,p,dir}=await setup(t);const files=await clips();const clip=await importMedia(files.mp4,'mp4',repo,{env:{}});const audio=await importMedia(files.wav,'wav',repo,{env:{}});
...
test('composition refuses missing narration and audio longer than the shot',async t =>{
  const {repo,p,dir}=await setup(t);const files=await clips();const clip=await importMedia(files.mp4,'mp4',repo,{env:{}});const s=p.video.shots[0];s.clip={...clip,source:shotSource(s,p.video.ratio,p)};s.narration='不能被省略的旁白';
...
test('WorkBuddy video handoff persists actual MP4, and cancellation preserves late output for explicit adoption',async t =>{
  const {repo,p}=await setup(t);const manager=createTaskManager(repo,{env:{}});const input=request(p,'video-shot',{provider:'workbuddy',objectId:'shot-a'});await manager.submit(input);const waiting=await until(manager,input.id,'waiting_external',task=>task.dispatch==='manual');
  ...
  const adopted=applyTaskResult(p,done);assert.ok(adopted.video.shots[0].clip.fileId.endsWith('.mp4'));await manager.stop();
  const manager2=createTaskManager(repo,{env:{}});const next=request(p,'video-shot',{provider:'workbuddy',objectId:'shot-a'});await manager2.submit(next);const second=await until(manager2,next.id,'waiting_external');await manager2.cancel(next.id);await writeFile(second.handoff.output,(await clips()).mp4);assert.equal((await until(manager2,next.id)).recoveredAfterCancel,true);await manager2.stop();
});
test('Runway sends documented inputs, preserves job ID on restart and never resubmits',async t =>{
  const {repo,p}=await setup(t);let posts=0,gets=0;const env={VIDEO_API_KEY:'test'};
...
test('sibling shot results remain adoptable, changed inputs reject stale results',async t =>{
  const {p}=await setup(t);p.video.shots.push(shot('shot-b'));const a=request(p,'video-shot',{objectId:'shot-a',provider:'workbuddy'});const b=request(p,'video-shot',{objectId:'shot-b',provider:'workbuddy'});
...
test('website compiler escapes content, validates references and exports an offline package',async t =>{
  const {repo,p,dir}=await setup(t,'website');const input=spec();input.pages[0].intro='<script>fetch("/api/workbench/data/workspace")</script>';
...
test('website request replaces new template generation while legacy files still rebuild',async t =>{
  const {repo,p}=await setup(t,'website');let calls=0;const manager=createTaskManager(repo,{env:{DEEPSEEK_API_KEY:'test'},fetchImpl:async()=>{calls++;throw new Error('No model needed to prepare the request');}});
  ...
  const rebuild=request(adopted,'website-build');await manager.submit(rebuild);const built=await until(manager,rebuild.id);assert.equal(calls,0);assert.match((await repo.output(built.result.website.previewFileId)).toString(),/改名后的旧网站/);assert.notEqual(built.result.website.previewFileId,p.website.previewFileId);await manager.stop();
});
test('output HTTP supports seek ranges, sandboxed HTML and hash checked backup restoration',async t =>{
  const {repo,dir,scope}=await setup(t,'website');const stored=await repo.putOutput(Buffer.from('<!doctype html><p>safe</p>'),'html');const bytes=(await clips()).mp4;const clip=await repo.putOutput(bytes,'mp4');
  const server=createRuntimeServer({dataDirectory:dir,env:{}});scope.defer(()=>closeTestServer(server));server.listen(0,'127.0.0.1');await once(server,'listening');const base='http://127.0.0.1:'+server.address().port;
...
test('composition mixes original sound with narration and accepts literal subtitle punctuation', async t => {
  const { repo, p, dir } = await setup(t);
...
test('regenerating video plans preserves explicit subtitle and original-audio settings', async t => {
  const { repo, p } = await setup(t); p.video.keepAudio = true; p.video.burnSubtitles = false;
  ...
  assert.equal(done.result.videoPlan.keepAudio, true); assert.equal(done.result.videoPlan.burnSubtitles, false); await manager.stop();
});
