...
async function setup(t){const scope=await createTestDirectory(t,'creative-workbench-test');const directory=scope.directory;const repo=createRepository(directory);return {directory,repo,scope};}
...
test('workspace commits are durable, idempotent and reject stale tabs and corrupt snapshots',async t =>{
  const{repo,directory}=await setup(t);const p=project();const ws=workspace(p);const id=randomUUID();
...
test('real images are decoded, immutable by hash and retained across reference removal',async t =>{
  const{repo}=await setup(t);const first=await repo.putImage(await png());
...
test('concurrent readers see complete task files while status is replaced',async t =>{
  const{repo}=await setup(t);const id=randomUUID();await repo.saveTask({id,step:0});
  await Promise.all([
...
test('a rejected WorkBuddy dispatch can finish through explicit manual handoff',async t =>{
  const{repo}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());
...
test('short story generation respects delivery notes and exports editable content',async t =>{
  const{repo}=await setup(t);const p=project();p.delivery.notes='结尾保留归还旧信的场景。';await repo.saveWorkspace(workspace(p),0,randomUUID());
...
test('text tasks persist before generation, deduplicate, and refuse stale adoption',async t =>{
  const{repo,directory}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());let calls=0;
...
test('section and selection edits only replace their targets, art and objects have validated outputs',async t =>{
  const{repo}=await setup(t);let p=project();p.content.novel.story='保留开头。修改中间。保留结尾。';p.requests[1]='温柔一点';
...
test('missing keys and malformed model output fail honestly without fabricated results',async t =>{
  const{repo}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());let calls=0;
...
test('uncertain delivery never triggers an automatic replacement request',async t =>{
  const{repo}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());let calls=0;
...
test('external edits upload the actual original image and carry the original style',async t =>{
  const{repo}=await setup(t);const image=await repo.putImage(await png());const p=project();p.art.fullPrompt='新的风格';p.assets=[{id:'original',name:'原图',fileId:image.fileId,source:{style:'原来的手绘风格'}}];
...
test('WorkBuddy sends the documented message, imports assigned output, and resumes after restart',async t =>{
  const{repo,directory}=await setup(t);const p=project();p.concepts=[{id:'object-1',category:'object',name:'葵扇',description:'用户提供的参考物件',prompt:'',revisionRequest:''}];await repo.saveWorkspace(workspace(p),0,randomUUID());let calls=0;
...
test('manual WorkBuddy handoff retains late pictures after cancellation without adopting them',async t =>{
  const{repo}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());const manager=createTaskManager(repo,{env:{}});
...
test('interrupted tasks become uncertain on restart and are not automatically resubmitted',async t =>{
  const{repo}=await setup(t);const p=project();await repo.saveWorkspace(workspace(p),0,randomUUID());const input=request(p,'art');
...
test('HTTP saves projects and media, and both server layers reject cross-origin writes',async t =>{
  const{directory,scope}=await setup(t);const server=createRuntimeServer({dataDirectory:directory,env:{}});scope.defer(()=>closeTestServer(server));server.listen(0,'127.0.0.1');await once(server,'listening');
  try{
