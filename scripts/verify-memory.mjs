...
const root=fileURLToPath(new URL('../work/',import.meta.url)),directory=join(root,'memory-checks',String(Date.now()));
...
const server=createRuntimeServer({dataDirectory:directory,env:{}});server.listen(0,'127.0.0.1');await once(server,'listening');const base='http://127.0.0.1:'+server.address().port;
...
try {
  ...
  const report={recordedAt:new Date().toISOString(),node:process.version,platform:process.platform,cycles:30,fixtures:{historicalTasks:96,imageTasks:48,imagePolls,zipMiB:16,rangeFileMiB:16,rangeRequests:120},samples,postWarmupHeapGrowthBytes:last.heapUsed-warm.heapUsed,maxSampledRss,
    acceptance:{cacheBounded:samples.every(s=>s.cache.weight<=8*1048576&&s.cache.entries<=64),postWarmupHeapGrowthBelow16MiB:last.heapUsed-warm.heapUsed<16*1048576,imagePollingCompleted:imagePolls===300},scope:'同一 Node 进程反复列任务、投影 48 个图片状态、复用/取消轮询器、创建 ZIP、选择性预览、HTTP Range；GC 后采样不是瞬时峰值，也不覆盖 WorkBuddy、浏览器或真实媒体供应商的长时运行。'};
...
}finally{poller.cancel();await new Promise(resolve=>server.close(resolve));}
