...
> **2026-09-10 美术提示词简化：** 一段当前提示词，直接编辑自动保存；AI 起草或修改后对照、替换，参考图片在下一步添加。旧画风内容与历史保留。协议 **14**、**31 工具**、Skills **0.11.0**。[交互与验证](docs/ART_PROMPT_EDITOR_2026-09-10.md)。

> **2026-09-10 器皿文化图案：** 网页可复用现有生图服务制作平面图案，本机自动贴到原模型外壁并下载 Blender/GLB；不需 HY 或 COS，保留内壁、附件和历史。协议 **13**、**31 工具**、Skills **0.10.0**。[流程与验收](docs/CRAFT_PATTERN_2026-09-10.md)。旧混元纹理路径保留，其 COS 配置见[旧接口说明](docs/CRAFT_TEXTURE_2026-09-10.md)。

> **2026-09-10 文字服务配置：** 支持自定义 API 地址与模型，新增腾讯云 Token Hub 按量配置预设。腾讯云密钥必须配合腾讯云接口使用；DeepSeek 官方接口返回 401 不能证明该密钥在腾讯云无效。创意、正文与 3D 造型方案共用配置。详见[文字服务接入](docs/TEXT_SERVICES_2026-09-10.md)。

> **2026-09-10 当前增量：独立 3D 资产生成。** 从一句需求开始，生成器具与简化建筑构件，在网页旋转查看并下载真实 Blender/GLB 文件。需要本机 Blender；网页可用已配置的文字服务自动解析，也可一次交给 WorkBuddy 接续原任务。无直接三维编辑，不支持任意雕塑。当前协议 **11**、**31 工具**、Skills **0.8.0**。使用、资源预算及实测范围见 [三维资产说明](docs/CRAFT_ASSETS_2026-09-10.md)。下文带日期的版本号与“3D 暂缓/仅前期资料”均为当时历史记录，以本入口说明为准。

> **2026-09-10 原任务交接修复：** 网页发起图片任务后，WorkBuddy 接续原任务 ID；已生成 PNG 可用 task_complete_handoff 写回原目录，图片入库不能代替交接完成。Skills 0.5.1、协议 7、24 工具，更新后重启 Runtime 并重新导入接入包。[修复与复测](docs/HANDOFF_CONTINUATION_2026-09-10.md)。
...
> **2026-09-09 功能核查与知识库：** 已修复双端同步、保存冲突与任务采用等问题，并加入首批 10 条可追溯岭南文化资料。页面“确定创意 → 选择文化资料”可选用；WorkBuddy 使用 knowledge_search / knowledge_apply。当前 Skills 0.5.1、核心协议 7，更新后重启 Runtime 并重新导入接入包。详见[核查结果](docs/FUNCTIONAL_AUDIT_2026-09-09.md)与[主题知识说明](docs/LINGNAN_KNOWLEDGE.md)。
...
> 项目名称：粤象·岭南文化创意工作台。本文是产品思路与功能方案，更新于 2026-09-08。
>
> **问题修复：** 当前问题处理、保留范围及回归说明见[2026-09-08 修复记录](docs/问题修复记录-2026-09-08.md)。

> **WorkBuddy 核心接入：** 已封装 2 项项目 Skills、24 个 stdio MCP 工具及本机连接包生成命令 `npm run workbench:setup`，新增只读 `prompt_prepare`。覆盖项目、文字、概念图、单镜头视频和网站生成任务交接。WorkBuddy 实际加载由用户手动测试；使用方法见 [核心接入与验收](docs/WORKBUDDY_CORE.md)。历史章节中的“MCP 只读清单”已由核心接入替代。
>
...
> **2026-09-08 历史开发顺序：** 当时暂缓 3D，优先完成现有多媒体生成器；现已进入上方说明的独立三维资产增量。第 1–14 节保留原有功能与界面设计背景，第 15 节及[多媒体生成器实施计划](docs/design/MULTIMEDIA_GENERATION_PLAN.md)记录当时排期。旧文化空间与直接编辑提案未随本次自动启用。

**粤象·岭南文化创意工作台以岭南文化为所有创作的共同背景，不面向脱离这一背景的任意创意。** 用户从一个想法出发，通过简单的几步操作，形成具有岭南文化语境的创作依据，最后交给 AI 一次发起生成完整作品。主题知识以独立检索并核对的官方文化资料为依据，保留来源、地域与事实/虚构区分；不默认导入《织梦者》的游戏设定或资产。各媒介通过共享文化依据实现复用。
...
```text
你是“粤象·岭南文化创意工作台”的创作助手。本工作台只服务于以岭南文化为背景的创作。
...
当前 CLI 只有发现和诊断，HTTP 支持健康检查、能力发现、创意服务状态与创意方案请求，MCP 只有只读 manifest。能力清单已登记 `creative-brief`，实际请求需要服务端密钥。`package.json` 中 `app-scaffold` 是技术占位，产品现名为“粤象·岭南文化创意工作台”；前端已按本次授权实现，技术包名保持不变，装饰性叶片图形不作为最终品牌标识。
