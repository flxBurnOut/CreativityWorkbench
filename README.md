...
> 暂定名称：创意工作台。本文是产品思路与功能方案，更新于 2026-09-07。
>
> **实现状态：P1 前端骨架与五阶段界面已实现，采用 B＋C 融合的岭南庭院美术方向。** 可新建与管理项目、编辑各阶段内容、上传图片、整理参考集，并在同一浏览器恢复草稿。正式服务端持久化、AI 生成、任务执行与真实成品交付尚未接入，不能将本轮视为完整 P1 或 P2–P5 已完成。当前实现与后续接入边界见第 13.8–13.9 节。

**创意工作台以岭南文化为所有创作的共同背景，不面向脱离这一背景的任意创意。** 用户从一个想法出发，通过简单的几步操作，形成具有岭南文化语境的创作依据，最后交给 AI 一次发起生成完整作品。
...
读取原始输入、已编辑方案与统一文化基础规则 → 调用文本模型 → 返回结构化内容 → 显示为可读文本 → 自动保存。
...
正式目标仍以服务端持久化为准，浏览器只缓存临时交互。本轮经确认先实现浏览器本地草稿作为 P1 前端基底，采用独立存储接口，尚未接入服务端项目存储。图片保存为受控文件，不把全部 Base64 塞进项目 JSON。删除和缩略图清理根据引用关系进行。
...
| P1 前端已完成，服务端待接入 | 项目卡、占位封面、五步导航、编辑与自动保存 | 已实现同一浏览器的新建、改名、编辑、图片暂存与恢复；完整验收仍需正式服务端持久化 |
...
当前 CLI 只有发现和诊断，HTTP 只有健康检查与空能力发现，MCP 只有只读 manifest。业务能力列表仍为空。`package.json` 中 `app-scaffold` 是技术占位，产品暂定名仍为“创意工作台”；前端已按本次授权实现，技术包名保持不变，装饰性叶片图形不作为最终品牌标识。
...
当前状态：**已选定 B＋C 融合方向，已实现完整前端流程与浏览器本地草稿；服务端持久化和生成能力仍待接入。**
...
| 创意 | 原始想法、创意方案、文化关联、修改要求均可编辑 | 创意完善与换方向 |
...
| 设置与帮助 | 服务状态、草稿说明、完整流程说明 | 服务端受保护的 API 配置；当前不收集或保存密钥 |
...
后续接入顺序：先将本地存储接口替换为服务端项目与图片持久化，补齐完整 P1；再接入文本、图片和成品适配器。外部 API 始终通过服务端，只有真实可用能力才进入 `workbench/manifest.json`。本轮未改动能力清单或 CLI/HTTP/MCP 行为。
